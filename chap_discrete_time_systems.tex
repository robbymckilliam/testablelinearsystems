...
\]
This is seen in Figure~\ref{fig:bandlimiteddiscretetimefiltering}.  The impulse response of this ideal lowpass filter $L_c$ is $2c\sin(2ct)$.
...
\begin{figure}[tp]
  ...
  \begin{tikzpicture}
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Fourier transform $\rect(f) - \cos(2\pi f) \big( \rect(2f - 3) + \tfrac{1}{2} \rect(2f + 3) \big)$ of a bandlimited signal with bandwidth $b < 2$ (dashed). The solid line shows the magnitude of the Fourier transform of the response to this signal of the ideal low pass filter with cuttoff frequency $c = 1$.  The spectrum of this filter is shown by the hatched rectangular pulse at the origin.  The higher frequency rasied cosine pulses centered at $f = \pm \tfrac{3}{2}$ are removed by the filter, but the rectangular pulse of height $\tfrac{3}{2}$ centered at the origin is unaffected.  The same affect is had by inputing this signal to an ideal lowpass digital filter with cuttoff $c=1$ and sample rate $F = \tfrac{1}{P} = 4$.  The shaded region shows the periodic magnitude spectrum of the filter.  The period is $F = 4$. The periodicity is not of consequence because the signal is bandlimited with bandwidth $b < \tfrac{F}{2} = 2$.} \label{fig:bandlimiteddiscretetimefiltering}
\end{figure}
