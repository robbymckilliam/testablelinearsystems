...
It follows from~\eqref{eq:transferLaplcetheorem} that if $H$ is a regular system with impulse response $h$ and spectrum $\Lambda H = \calL h$ and if $x \in \dom h$ is a signal with Fourier transform $\calF x$, then the signal $H x$ has Fourier transform
\[
