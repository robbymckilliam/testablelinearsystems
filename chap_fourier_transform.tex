...
\int_{-\infty}^{\infty} x(t)y^*(t) dt = \int_{-\infty}^{\infty} \hat{x}(f)\hat{y}^*(f) dt \qquad \text{(Exercise~\ref{exer:parsevalconsq})}
\end{equation}
...
\[
\calF( \hat{x} ) = \calF( \calF(x)) = \calF^2(x) = x(-t) \qquad a.e.
\]
...
\begin{align}
\hat{x}(f) = \calF x(f) &= \calF\left( \sum_{n \in \ints} c_n \sinc(F t - n)\right)(f) 
&= \sum_{n \in \ints} c_n \calF(\sinc(F t - n))(f)  \label{eq:ftanddftrelationshipswapsumf} \\
...
\]
This is analogous to how the Fourier transform of a square integrable signal was assigned using the Cauchy principal value~\eqref{eq:cauchyprinvalint}.  The relationship~\eqref{eq:ftanddftrelationship} between $\hat{x}$ and $\hat{c} = \calD c$ still holds in this case provided that equality is weakened to equality almost everywhere (Exercise~\ref{exer:ftdftswapsummationldct}).% that is, if $c$ is square integrable, then $\hat{x}(t) = P \rect( f P ) \calD c(Pf)$
