...
\end{cases}
\]
The function $f$ maps Mario to Conker, Link to Sephiroth, and Ness to Sephiroth.  The set $X$ is called the \term{domain} of the function $f$ and the set $Y$ the \term{range}.  The value of $f$ for input $x$ is denoted by $f(x)$ so, for example, $f(\text{Mario}) = \text{Conker}$ and $f(\text{Link}) = \text{Sephiroth}$.  The set of all functions mapping $X$ to $Y$ is denoted by $X \to Y$ and so $f \in X \to Y$ in the example above.

A \term{signal} is a function with domain the set of real numbers $\reals$ and range the set of complex numbers $\complex$, that is, a signal is a function from the set $\reals \to \complex$.  For example
\[
...
\end{cases}
\end{equation}
is right-sided.  Its horizontal reflection $u(-t)$ is left sided (Figure~\ref{fig:stepsided}).  A signal $x$ is said to have \term{finite support} if it is both left and right sided, that is, if there exits a $T\in\reals$ such that $x(t) = x(-t) = 0$ for all $t > T$.  The signals $\sin( \pi t)$ and $e^{-t^2}$ do not have finite support, but the \term{rectangular pulse}
...
\]
for all finite constants $a$ and $b$, where $< \infty$ means that the integral evaluates to a finite number.  The signals $\sin(\pi t)$, $\tfrac{1}{2}t^3$, and $e^{-t^2}$ are all locally integrable.  An example of a signal that is not locally integrable is $x(t) = \frac{1}{t}$ (Exercise~\ref{exer:oneontnotlocallyint}).  
...
\]
for all $t \in \reals$ if $x$ is locally integrable.  So another possible domain for $I_\infty$ is the set of right sided locally integrable signals.  The the union of $L^1$ and the set of right sided locally integrable signals is another possible domain.
...
\]
Figure~\ref{fig:timescaler} depicts the action of time-scalers with different values for $\alpha$.  When $\alpha=-1$ the time-scaler reflects the input signal in the $t$ axis.  When $\alpha = 1$ the time-scaler is the identity system $T_0$.  Both the shifter and time-scaler are well defined for all signals and so it is reasonable to choose their domains as the entire set of signals $\reals \to \complex$.
...
The differentiator is only defined for differentiable signals.  The largest possible domain for $D$ is the set of differentiable signals.
...
\]
and so the linear combination $ax + by$ is even.  The set of absolutely integrable signals $L^1$ and the set of square integrable signals $L^2$ are linear spaces (Exercise~\ref{exer:L1L2linearshiftinvariant}).  The set of periodic signals is not a linear space (Exercise~\ref{exer:periodicshiftinvariantnotlinear}).

A set of signals $X$ is \term{shift-invariant} if for all signals $x \in X$ and all real numbers $\tau$ the shifted signal $T_\tau x$ is also in $X$.  The set of differentiable signals is shift invariant because, if $x$ is differentiable, then $T_\tau x$ is differentiable.  The derivative is $T_\tau D x$.  The set of periodic signals is shift invariant (Exercise~\ref{exer:periodicshiftinvariantnotlinear}) as are $L^1$ and $L^2$~\eqref{exer:L1L2linearshiftinvariant}.  The set of even signals and the set of odd signals are not shift invariant.
...
A system $H \in X \to Y$ is called \term{memoryless} if, for all input signals $x \in X$, the output signal $Hx$ at time $t$ depends only on $x$ at time $t$.  For example $\frac{1}{x(t)}$ and the identity system $T_0$ are memoryless, but 
\[
...
Let $H \in X \to Y$ be a system with domain $X$ and range $Y$ being linear spaces.  The system $H$ is \term{linear} if
\[
H( ax + by) = a Hx + b Hy
\]
...
Let $H \in X \to Y$ be a system with shift-invariant domain $X$ and shift-invariant range $Y$. The system $H$ is \term{shift-invariant} if
\[
H T_\tau x(t) = Hx(t-\tau)
\]
