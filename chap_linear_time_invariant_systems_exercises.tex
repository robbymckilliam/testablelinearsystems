...
&= \int_{-\infty}^\infty \abs{\int_{-\infty}^\infty x(\tau) y(t - \tau) d\tau } dt \\
 ...
 &= \int_{-\infty}^\infty \int_{-\infty}^\infty \abs{ x(\tau)  }{ y(t - \tau) } dt d\tau \qquad \text{(change order integration (Tonelli's theorem))} \\
 &= \int_{-\infty}^\infty \int_{-\infty}^\infty { y(t - \tau) } dt \abs{ x(\tau) } d\tau \\
  &= \|y\|_1 \int_{-\infty}^\infty \abs{ x(\tau) } d\tau \\
  &= \|y\|_1 \|x\|_1
\end{align*}
which is finite by our assumption that $x$ and $y$ are absolutely integrable.  This result also follows as a special case of Young's Theorem~\cite{Rudin_real_and_complex_analysis}.
\end{solution}
