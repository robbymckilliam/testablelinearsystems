...
An \term{ideal high pass digital filter} with cuttoff frequency $c$ and period $P = \tfrac{1}{F} < \tfrac{1}{2c}$ is the system given by the linear combination 
\[
T_0 - L_c^{P}.
\]  
The impulse response is $\delta_n - 2cP\sinc(2cnP)$.  The spectrum 
\[
\Lambda T_0 - \Lambda L_c^P = 1 - \Lambda L_c^{P}
\]
is plotted in Figure~\ref{fig:bandlimiteddiscretetimefiltering} (middle) in the case that $c = 1$ and $F = \tfrac{1}{P} = 4$.  Because $F > 2c$ the spectrum is a periodic sum of rectangular pulses of width $2(F-c)$ centered at multiples of $F/2$.  That is
\[
1 - \Lambda L_c^{P} = \sum_{m \in \ints} \rect\left( \frac{f + F/2 + Fm}{2(F-c)}  \right).
\]  
...
An \term{ideal bandpass digital filter} with upper cuttoff frequency $u$ and lower cuttoff frequency $\ell$ is the system given by the linear combination 
\[
L_u^P - L_\ell^P.
\]  
The impulse response is the sequence $2uP\sinc(2unP) -  2 \ell P\sinc(2\ell nP)$.  The spectrum is plotted in Figure~\ref{fig:bandlimiteddiscretetimefiltering} (bottom) in the case that $u = \tfrac{3}{2}$, $\ell = \tfrac{1}{4}$, and $F = \tfrac{1}{P} = 4$.  The Fourier transform of the response of $L_u^P - L_\ell^P$ to input $x$ from~\eqref{eq:examplebandlimtedsignalforfiltering} is also plotted.
