...
\end{equation}
where $n \geq 0$ is an integer (Exercise~\ref{excer:laplacetransformcommonpolyut}).  The Laplace transforms of the signal $x(t)$ and the signal $e^{\alpha t}x(t)$ are related.  The region of convergence of $e^{\alpha t}x(t)$ is all those complex numbers $s$ such that $s - \alpha \in \roc(x)$ and the Laplace transform satisfies
\begin{align}
