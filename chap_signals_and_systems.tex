...
\lim_{h \to 0} u(h) = 1 \neq 0 = \lim_{h \to 0} u(-h).
\]
The set of continuous signals is denoted by $C^0(\reals)$ or just $C^0$.  A signal $x$ is \term{continuously differentiable} or just \term{differentiable} if
\[
...
The differentiator is only defined for differentiable signals.  The largest possible domain for $D$ is the set of differentiable signals $C^1$ and the largest possible domain for $D^k$ is the set of $k$-times differentiable signals $C^k$.
