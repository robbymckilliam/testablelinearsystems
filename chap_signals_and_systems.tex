...
\end{cases}
\]
The function $f$ maps Mario to Conker, Link to Sepiroth, and Ness to Sephiroth.  The set $X$ is called the \term{domain} of the function $f$ and the set $Y$ a \term{range}.  The set of all functions mapping $X$ to $Y$ is denoted by $X \to Y$ and so $f \in X \to Y$.

A \term{signal} is a function that maps each element from the set of real numbers $\reals$ to elements from the set of complex numbers $\complex$, that is, a function from $\reals \to \complex$.  For example
\[
\sin( \pi t), \qquad \frac{1}{2} t^3, \qquad e^{-t^2}
\]   
all represent signals with $t \in \reals$.  These signals are plotted in Figure~\ref{fig:signalsstart}.  Many physical phenomena, such as sound, light, weather, and motion, can be modelled using signals.  In this text we primarily focus on examples from electrical and mechanical engineering where signals are use to model changes in voltage, current, position, and angle over time.  In these examples, the independent variable $t$ will represent time.  However, there is no fundamental reason for this and the techniques we devolop can be applied equally well when $t$ represents a quantity other than time.  Any example where this occurs is in image processing.
...
\]
Considered a function of $t$ the value of the limit above is called the derivative of $x$ at $t$ and is denoted by $\frac{d}{dt} x(t)$.  For example, the signals $\sin( \pi t)$, $\frac{1}{2} t^3$, $e^{-t^2}$ and $t^2$ are differentiable with derivatives
\[
...
The set of all differentiable signals forms a linear space because if $x$ and $y$ are differentiable signals then the linear combination $ax + by$ is differentiable.  The derivative is $a \frac{d}{dt} x + b \frac{d}{dt} y$.
...
%\footnote{It is customary to drop the brackets only where $H$ is a \term{linear} system (Section~\ref{sec:properties-systems}).  We deal primarily with linear systems in this text and so droping the brackets will rarely be a Faux Pas.}.  
...
\begin{figure}[p]
  ...
  \end{tikzpicture}
  \caption{Scaler system $Hx(t) = x(\alpha t)$ for $\alpha=-1,\tfrac{1}{2}, 1$ and $2$ acting on the signal $x(t) = e^{-(t-2)^2}$.} \label{fig:timescaler}
\end{figure}
...
We must be carefull when choosing a domain for the differentiator.  For example, consider the step function $u(t)$ from~\eqref{eq:stepfunction}.  We cannot differentiate $u(t)$ at $t = 0$ because
\[
