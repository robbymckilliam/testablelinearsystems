...
\]
We take $\roc x$ as the domain of $\calL x$ and so the Laplace transform is a complex valued function of its region of convergence, that is, $\calL x \in \roc x \to \complex$.
...
Recall from Section~\ref{sec:eigenf-line-time} that complex exponential signals are \term{eigenfunctions} of linear shift-invariant systems.  That is, if $H \in X \to Y$ is a linear shift-invariant system then the response of $H$ to the complex exponential signal $e^{st} \in X$ satisfies $H e^{st} = \lambda H(s) e^{st}$ where $\lambda H \in \cep X \to \complex$ is the transfer function of $H$.  The domain $\cep X \subseteq \complex$ of the transfer function is the set of complex numbers such that the signal $e^{st}$ is inside the domain of the system $H$, that is, such that $e^{st} \in X$.

Let $H \in \dom h \to Y$ be a regular system with impulse response $h$.  The complex exponential signal $e^{st} \in \dom h$ if and only if $s \in \roc h$, that is, $\roc h = \cep \dom h$ (Exercise~\ref{exer:finhandroch}).  The response of $H$ to $e^{st} \in \dom h$ satisfies
\begin{align*}
H e^{st} = \lambda H(s) e^{st} &= h * e^{st} \\
&= \int_{-\infty}^{\infty} h(\tau) e^{s(t - \tau)} d\tau \\
&= e^{st} \int_{-\infty}^{\infty} h(\tau) e^{-s \tau} d\tau \\
&= e^{st} \calL h(s)
\end{align*}
and so $\lambda H  = \calL h$.  That is, the transfer function of a regular system is precisely the Laplace transform of its impulse response.  Observe that the domains of $\lambda H$ and $\cal h$ are the same, that is, $\cep\dom h = \roc h$.  %The region of convervence of the Laplace transform $\roc(h)$ corresponds with the domain of the regular system $\dom h$.  %The region of convergence of the impulse response describes a set of complex exponential signals $e^{st}$ in the domain of the system and we refer to this as the region of convergence of the \emph{system}.  In this way, both signals and systems have regions of convergence.
...
Let $F \in X \to Y$ and $G \in X \to Y$ be linear shift-invariant systems and let $H = a F + bG$ be the system formed by a linear combination of $F$ and $G$.  The response of $H$ to the complex exponential signal $e^{st} \in X$ is
\begin{align*}
H e^{st} &= a F e^{st} + b G e^{st} \\
&= a\lambda F(s) e^{st} + b\lambda G(s) e^{st}  \\
&= \big( a\lambda F(s) + b\lambda G(s) \big) e^{st} \\
&= \lambda H(s)  e^{st} \qquad s \in \cep X
\end{align*}
and so $\lambda H = a\lambda F + b\lambda G$.  That is, the transfer function of a linear combination of systems is the same linear combination of the transfer functions.  We can take the domain of $\lambda H$ to be $\cep X$.  %The region of convergence of the linear combination is the intersection of the regions of convergence of the systems being combined.

Now let $F \in X \to Y$ and $G \in Y \to Z$ be linear shift-invariant systems and let $H \in X \to Z$ be the system formed by the composition $H = G F$.  The response of $H$ to the signal $e^{st} \in X$ is
\[
H e^{st} = G F e^{st} = G \big(\lambda F(s) e^{st} \big) = \lambda F(s) G e^{st}
\]
We will only require the case when $\cep X \subset \cep Y$.  In this case $e^{st} \in Y$ and 
\[
H e^{st} = \lambda F(s) \lambda G(s) e^{st} = \lambda H(s) e^{st}.
\]
It follows that
\begin{equation}\label{eq:composedtransferfunction}
\lambda H(s) = \lambda F(s) \lambda G(s) \qquad $s \in \cep X \subset \cep Y$
\end{equation}
...
Suppose that $F$ and $G$ are also regular systems with impulse responses $f$ and $g$ and that the domain of $F$ is $X \in \dom g\;f$ and the domain of $G$ is $Y = \dom g$.  We showed in Section~\ref{sec:line-comb-comp} that the composition $H = G F$ is a regular system with impulse response given by the convolution $h = f * g$.  Because,
\[
\lambda H = \calL h \qquad \lambda F = \calL f \qquad \lambda G = \calL g,
\] 
and using~\eqref{eq:composedtransferfunction}, we obtain,
\[
\calL(f * g) = \calL h =  \lambda H = \lambda F \lambda G = \calL f \calL g
\]
that holds for $s \in \dom f \cap \dom g = \roc f \cap \roc g$.  Putting $x = f$ and $y = g$, we obtain the \term{convolution theorem},
\begin{equation}\label{eq:convtheoremlaplace}
\calL(x*y) = \calL x \calL y \qquad s \in \roc x \cap \roc y.
\end{equation}
In words: the Laplace transform of a convolution of signals is the multiplication of their Laplace transforms.  The domain of $\calL(x * y)$ is the intersection of the regions of convergence of $\calL x$ and $\calL y$, that is, $\roc x \cap \roc y$.  
