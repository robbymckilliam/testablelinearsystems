...
Let $H \in \fin h \to Y$ be a regular system with impulse response $h$.  The complex exponential signal $e^{st} \in \fin h$ if and only if $s \in \roc h$, that is, $\roc h = \cep \fin h$ (Exercise~\ref{exer:finhandroch}).  The response of $H$ to $e^{st} \in \fin h$ satisfies
\begin{align*}
...
These results motivate assigning the following Laplace transforms to the delta ``function'', its shift $T_\tau \delta = \delta(t - \tau)$, and the $\delta^k$ symbol,
\[
...
\]
that holds for $s \in \fin h_1 \cap \fin h_2 = \roc h_1 \cap \roc h_2$.  Putting $x = h_1$ and $y = h_2$, we obtain the \term{convolution theorem},
