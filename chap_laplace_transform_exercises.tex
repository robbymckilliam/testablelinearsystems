...
\item \label{exer:finhandroch} Let $H$ be a regular system with impulse response $h$.  Show that the complex exponential signal $e^{st} \in \dom h$ if and only if $s \in \roc h$, that is, $\roc h = \cep \dom h$.
\begin{solution}
...
\]
The set $\dom h$ contains all those signals such that
\[
\int_{-\infty}^{\infty} \abs{h(\tau) x(t - \tau} d\tau < \infty  \qquad \text{for all $t \in \reals$}.
\]
The set $\cep \dom h$ contains the complex numebrs $s$ such that the complex exponential signal $e^{st} \in \dom h$, that is, those complex $s$ such
\[
