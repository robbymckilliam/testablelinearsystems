...
It is convenient to choose period of the filter to be $P = \tfrac{1}{F}$ where $F = \SI{22050}{\hertz}$ is the sample rate of the audio recording.  The cuttoff frequency is chosen to be $\gamma = \SI{4800}{\hertz}$ and a Blackman window~\eqref{eq:blackmanwindow} of width $W=\tfrac{10}{\gamma} = \tfrac{1}{480}$ is used.  This filter has impulse response
\[
h_n = 2 \gamma P w(t) \sinc( 2\gamma P n)
\]
...
\] 
This filter has $2a+1 = 45$ taps.  Denote this low pass finite impulse response filter by the discrete time system $L_\gamma^{P,w}$.

Let 
\[
x(t) = \sum_{n \in \ints} c_n \sinc(F t - n)
\]
be the bandlimited signal reconstructed from the sequence $c$.  The response of the low pass filter $L_c^{P,w}$ to input $x$ is
\begin{align*}
L_c^{P,w}x(t) &= \sum_{m \in \ints} h_m T_{Pm}x(t) \\
&= \sum_{m = -a}^a h_m T_{Pm}x(t) \\
&= \sum_{m = -a}^a h_m T_{Pm} \left( \sum_{n \in \ints} c_n \sinc(F t - n) \right) \\
&= \sum_{m = -a}^a \sum_{n \in \ints} h_m  c_n T_{Pm} \sinc(F t - n)  \\
&= \sum_{n \in \ints} \sum_{m = -a}^a h_m  c_n T_{Pm} \sinc(F t - n)  \\
\end{align*}
