...
%BLERG:  Potentially two sections "Ideal analogues filters" and "Butterworth filters". 
...
A \term{sequence} is a function with domain given by the integers $\ints$.  The value of a sequence $x$ at integer $n$ can be denoted by $x(n)$ but it is conventional to write $x_n$.  We are primarily interested in sequences that take complex values, that is, functions from the set $\ints \to \complex$.  For example,
\[
...
A sequence $x$ is \term{right sided} if there exists a $T \in \reals$ such that $x_n = 0$ for all $n < T$.  Correspondingly $x$ is \term{left sided} if $x_n = 0$ for all $n > T$.  For example, the \term{step sequence} $u$ with $n$th element 
...
\end{cases}
\end{equation}
is right sided  (Figure~\ref{fig:stepsided}).  The reflected sequence $u_{-n}$ is left sided.  A sequence is said to be of \term{finite support} or just \term{finite} if it is both left and right sided.  For example the the \term{delta sequence} $\delta$ with $n$th element 
...
where $c$ is a square integrable complex valued sequence and $F = 2b$.  This is a consequence of a property of the set of square integrable signals $L_2$ called \term{completeness}~\cite[Theorem~3.11 and page~91]{Rudin_real_and_complex_analysis}.  The sum on the right hand side of~\eqref{eq:sincinterformula} is often called \term{sinc interpolation}.  %This property is sometimes also called the \term{Riesz-Fischer theorem}~\cite[page~91]{Rudin_real_and_complex_analysis}.
...
\end{equation}
is called the~\term{discrete-time Fourier transform} of the sequence $c$.  The interchange of Fourier transformation and summation on line~\eqref{eq:ftanddftrelationshipswapsumf} is justified by \term{Lebesgue's dominated convergence theorem}~\cite[Section~1.34]{Rudin_real_and_complex_analysis} (Exercise~\ref{exer:ftdftswapsummationldct}).  We write $\hat{c} = \calD c$ for the discrete-time Fourier transform of $c$.  The discrete-time Fourier transform $\hat{c} = \calD c$ is a periodic function from $\reals \to \complex$, that is, $\hat{c}$ is a periodic signal with period $1$.  For example, the discrete-time Fourier transform of the delta sequence is 
\[
...
\begin{tikzpicture}
  \begin{scope}[xscale=2,yscale=0.75]
    \draw[->] (-1.35,0) -- (1.35,0) node[above] {$f$};
    \draw[->] (0,-1.066) -- (0,3.466) node[above] {$\calD\big(\rect(n/2)\big) = 1 + 2\cos(2\pi f)$};
...
Replacing $Pf$ with $f$ in~\eqref{eq:ftanddftrelationship} and multiplying by $F$ we obtain
\[
...
%  \def\ifthenelset(#1,#2,#3){(#1*#2 + !#1*#3)}
  \def\sincfunc(#1){(sin(\PI*(#1))/(\PI*(#1)))}
%  \def\sincfunczero(#1){\ifthenelset((abs(#1) < 0.01), 1.0, \sincfunc(#1))}
  \begin{figure}[t]
    \centering
    \begin{tikzpicture}[domain=\minn:\maxn]
      \begin{scope}[yscale=\scaley,xscale=\scalex]
        \draw[->] (-2.8/\scalex,0) -- (2.8/\scalex,0) node[above] {$t$};
        \draw[->] (0,-0.4) -- (0,1.3) node[above] {$\sinc(n/3)$};
     \draw[color=black,thick,ycomb,mark=*,mark options={xscale=1/\scalex,yscale=1/\scaley,scale=0.75},samples=\maxn-\minn+1] plot function{\sincfunc(x/3.0)};
     \draw[thick] (0,0) -- (0,1);
     \end{scope}
     \draw[fill=black] (0,\scaley) circle (0.06);
      \begin{scope}[yscale=\scaley]
        \htick{1} node[pos=0.5,left] {$1$};
      \end{scope}
      \begin{scope}[xscale=\scalex]
...
\newcommand{\drawrectanglepeice}[1]{(#1-\bandwidth,0)--(#1-\bandwidth,1)--(#1+\bandwidth,1)--(#1+\bandwidth,0)}
  \begin{tikzpicture}[yscale=\scaley,domain=-2.4:2.4,samples=500]
    ...
    \htick{1} node[pos=0.5,above left] {$3$};
...
%      \vtick{-2} node[pos=0.5,below] {$-2F$};
    \end{scope}
  \end{tikzpicture} 
    \caption{The sequence $\sinc(n/3)$ and its discrete-time Fourier transform $3 \sum_{m\in\ints}\rect(3f+3m)$.} \label{fig:sincsampleddft}
  \end{figure}
}

Substituting $Pf$ for $f$ in~\eqref{eq:dftsignalperiodicsumP} and multiplying by $P$ we find that
\[
...
\centering
{
  ...
  \begin{tikzpicture}[yscale=\scaley,domain=-2.4:2.4,samples=500]
    \draw[->] (-2.8,0) -- (2.8,0) node[above] {$t$};
    \draw[->] (0,-0.7/\scaley) -- (0,2.4/\scaley) node[above] {$\rect(3t/2)$};
...
\vspace{1cm}
{
  ...
  \begin{tikzpicture}[yscale=\scaley,domain=-2.4:2.4,samples=100]
    \draw[->] (-2.8,0) -- (2.8,0) node[above] {$t$};
    \draw[->] (0,-0.35) -- (0,1.4) node[above] {$e^{-\pi t^2}$};
    \draw[smooth,color=black,thick] plot function{\f(x)};
    % \htick{1} node[pos=0.5,above left] {$1$};
    \begin{scope}[yscale=1/\scaley]
      \vtick{1} node[pos=0.5,below] {$1$};
...
\vspace{1cm}
{
  ...
  \begin{tikzpicture}[yscale=\scaley,domain=-2.4:2.4,samples=100]
    \draw[->] (-2.8,0) -- (2.8,0) node[above] {$t$};
    \draw[->] (0,-0.7) -- (0,0.7) node[above] {$t \rect(\tfrac{t}{2})\big(1 + \cos(\pi t)\big)$};
    \draw[smooth,color=black,thick] plot function{\f(x)};
    \htick{0.5} node[pos=0.5,left] {$\tfrac{1}{2}$};
    \begin{scope}[yscale=1/\scaley]
      \vtick{1} node[pos=0.5,below] {$1$};
      \vtick{-1} node[pos=0.5,above] {$-1$};
 %     \vtick{2} node[pos=0.5,below] {$2F$};
...
\caption{Signals $x$ and their periodic summations $\sum_{m\in\ints}x(t + m)$. The periodic summation is shown by the solid lines on the left.  The dashed lines are the individual translations being summed.} \label{fig:periodisedfunctions}
\end{figure}
...
\centering
{
  ...
  \begin{tikzpicture}[xscale=\scalex,yscale=\scaley,domain=-2.5/\scalex:2.5/\scalex,samples=100]
    \draw[->] (-2.8/\scalex,0) -- (2.8/\scalex,0) node[above] {$t$};
    \draw[->] (0,-0.35) -- (0,1.4) node[above] {$x(t) = e^{-\pi F^2 t^2/2}$};
  ...
  \begin{tikzpicture}[yscale=\scaleyF,domain=-2.4:2.4,samples=100]
    \draw[->] (-2.8,0) -- (2.8,0) node[above] {$f$};
    \draw[->] (0,-0.35*1.4142) -- (0,1.4*1.4142) node[above] {$\hat{x}(f) = \sqrt{2}Pe^{-2\pi P^2 t^2}$};
    \draw[smooth,color=black,thick] plot function{\Fexpsig(x)};
    \htick{1.41412} node[pos=0.5,above left] {$\sqrt{2} P$};
    \begin{scope}[yscale=1/\scaley]
      \vtick{1} node[pos=0.5,below] {$F$};
      \vtick{-1} node[pos=0.5,below] {$-F$};
      %\vtick{2} node[pos=0.5,below] {$2F$};
 ...
 \\
 \vspace{1cm}
  \begin{tikzpicture}[xscale=\scalex,yscale=\scaley,domain=-3:3]
    \draw[->] (-2.8/\scalex,0) -- (2.8/\scalex,0) node[above] {$t$};
    \draw[->] (0,-0.35) -- (0,1.4) node[above] {$c_n = x(nP) = e^{-\pi n^2/2}$};
    \draw[color=black,thick,ycomb,mark=*,mark options={xscale=1/\scalex,yscale=1/\scaley,scale=0.75},samples=7] plot function{\expsig(x)};
    \htick{1} node[pos=0.5,above left] {$1$};
    \begin{scope}[yscale=1/\scaley]
      \vtick{1} node[pos=0.5,below] {$1$};
      \vtick{-1} node[pos=0.5,below] {$-1$};
  ...
  \end{tikzpicture} 
  \;\;
  \begin{tikzpicture}[yscale=\scaleyF,domain=-2.5:2.5,samples=100]
    \draw[->] (-2.8,0) -- (2.8,0) node[above] {$f$};
    \draw[->] (0,-0.35*1.4142) -- (0,1.4*1.4142) node[above] {$\hat{c}(f) = F \sum_{m \in \ints} \hat{x}(Ff + Fm)$};
    \draw[smooth,color=black,dashed] plot function{\Fexpsig(x-3)};
    \draw[smooth,color=black,dashed] plot function{\Fexpsig(x-2)};
    \draw[smooth,color=black,dashed] plot function{\Fexpsig(x-1)};
    \draw[smooth,color=black,dashed] plot function{\Fexpsig(x)};
    \draw[smooth,color=black,dashed] plot function{\Fexpsig(x+1)};
    \draw[smooth,color=black,dashed] plot function{\Fexpsig(x+2)};
    \draw[smooth,color=black,dashed] plot function{\Fexpsig(x+3)};
    \draw[smooth,color=black,thick] plot function{\Fexpsig(x-3)+\Fexpsig(x-2)+\Fexpsig(x-1)+\Fexpsig(x)+\Fexpsig(x+1)+\Fexpsig(x+2)+\Fexpsig(x+3)};
    \htick{1.41412} node[pos=0.5,above left] {$\sqrt{2}$};
    \begin{scope}[yscale=1/\scaley]
      \vtick{1} node[pos=0.5,below] {$1$};
      \vtick{-1} node[pos=0.5,below] {$-1$};
      %\vtick{2} node[pos=0.5,below] {$2F$};
 ...
 \\
  ...
  \begin{tikzpicture}[xscale=\scalex,yscale=\scaley,domain=-2.5/\scalex:2.5/\scalex,samples=100]
    \draw[->] (-2.8/\scalex,0) -- (2.8/\scalex,0) node[above] {$t$};
    \draw[->] (0,-0.35) -- (0,1.4) node[above] {$\sum_{n \in \ints} c_n \sinc(Ft - n)$};
    \draw[smooth,color=black,thick] plot function{(\expsig(-2)*\sincfunc(\F*x+2)) + (\expsig(-1)*\sincfunc(\F*x+1)) + (\expsig(0)*\sincfunc(\F*x)) + (\expsig(1)*\sincfunc(\F*x-1)) + (\expsig(2)*\sincfunc(\F*x-2))};
%     \draw[smooth,color=black,thick] plot function{(\expsig(0)*\sincfunc(x))};
    \htick{1} node[pos=0.5,above left] {$1$};
    \begin{scope}[yscale=1/\scaley]
      \vtick{1} node[pos=0.5,below] {$P$};
      \vtick{-1} node[pos=0.5,below] {$-P$};
  ...
  \end{tikzpicture} 
  \;\;
  \begin{tikzpicture}[yscale=\scaleyF]
    \draw[->] (-2.8,0) -- (2.8,0) node[above] {$f$};
    \draw[->] (0,-0.35*1.4142) -- (0,1.4*1.4142) node[above] {$P \rect( f P ) \hat{c}(Pf)$};
    \draw[color=black,thick] (-2.5,0) -- (-0.5,0) -- plot[smooth,domain=-0.5:0.5,samples=50] function{\Fexpsig(x)}-- (0.5,0) -- (2.5,0);
    \htick{1.41412} node[pos=0.5,above left] {$\sqrt{2} P$};
    \begin{scope}[yscale=1/\scaley]
      \vtick{1} node[pos=0.5,below] {$F$};
      \vtick{-1} node[pos=0.5,below] {$-F$};
...
Because of this it is sufficient to know only $(\calD_N c)_k$ for $k = 0,\dots,N-1$ in order to know the entire sequence $\calD_N c$.  Given $d = \calD_N c$ the original samples $c_0, \dots, c_{N-1}$ can be recovered by 
\[
