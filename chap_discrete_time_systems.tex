...
\end{equation}
where $h \in \ints \to \complex$ is a complex valued sequence.  Such systems are called \term{discrete-time systems}.  
...
\end{equation}
The sum is called the \term{discrete convolution} of sequences $h$ and $c$.  The notation $h * c$ is reused to denote discrete convolution so that $d = h * c$.
We can recover this definition from~\eqref{eq:discretimesysdefn} by identifying $d_n$ with $Hx(nP)$ and $c_n$ with $x(nP)$.  %In this way,~\eqref{eq:discretimesysdefn} is a more general definition of discrete-time system.
We will find that~\eqref{eq:discretimesysdefn} provides a setting for the study of discrete-time systems that is closely linked with previous chapters where what are often called \term{continuous-time} systems have been studied.  Complex exponetial signals $e^{st}$ remain the eigenfunctions and the notions of the transfer function and spectrum carry through unchanged.  This definition of discrete-time system~\eqref{eq:discretimesysdefn} is connected with what \citet[Sec.~9.5]{Zemanian_dist_theory_1965} calls the continuous variable case of a linear \term{difference equation} with constant coefficients.  We will study difference equations in Section~\ref{sec:difference-equations}.
...
% 
% Discrete-time systems have particularly convenient properties when operating upon bandlimited signals.  Let $x$ be a signal with bandwidth $b$ and let $c_n = x(nP)$ be the sequence of samples of $x$ at rate $F = \tfrac{1}{P} > 2b$.  Recall from Section~\ref{sec:bandlimited-signals} that the bandlimited signal $x$ can be recovered from the sequence of samples $c$ by sinc interpolation
% \[
% x(t) = \sum_{n \in \ints} c_n \sinc(F t - n).
% \]
% For this reason, when working with bandlimited signals, it is enough to know only the sequence of samples $c$.  Now suppose that $H$ is a discrete-time system with impulse response $h$ and sample period $P$ and suppose that the bandlimited signal $x \in \dom_P h$.  We will regularly be interested in computing the response $y = Hx$ and it will often be that we only require the samples $d_n = y(nP) = Hx(nP)$ of the response at period $P$ (see Test~\ref{} for example).  The samples take the form
% \begin{align*}
% d_n &= y(nP) = Hx(nP) \\
% &= \sum_{m \in \ints} h_m T_{mP}x(nP) \\
% &= \sum_{m \in \ints} h_m x(nP - mP) \\
...
% \end{align*}
% that is, the sequence $d = h * c$. The sequence $d$ of samples of the response $y = Hx$ is the discrete convolution of the impulse response $h$ and the sequence $c$ of sample of the input signal $x$.  Observe that in computing 


%To see why, let $H$ be a discrete-time system with impulse response $h$ and sample period $P$.  Let $x \in \dom_P h$.  Suppose that we are only interested in the samples of the response $Hx$, that is, suppose    and let $x$ be a bandlimited signal with bandwidth $b \leq \tfrac{F}{2} = \tfrac{1}{2P}$.  Let $c_n = x(nP)$ be the sequence of samples of $x$ so that
% \[
% x(t) = \sum_{n \in \ints} c_n \sinc(Ft - n).
% \]
% %that is, $x$ my be recovered by $\sinc$ interpolation
% The response of $H$ to $x$ is
% \begin{align*}
% y = Hx &= \sum_{n \in \ints} h_n T_{Pn}\left(\sum_{m \in \ints} c_m \sinc(Ft - m) \right) \\
% &=  \sum_{m \in \ints} \sum_{n \in \ints} h_n c_m \sinc(Ft - n - m),
% \end{align*}
% the interchange of summation being valid under appropriate assumptions about the sequences $h$ and $c$.  Putting $k = m+n$ we have
% \[
...
\]
The domain of $\lambda T_0$ is the entire complex plane $\complex$.  This agrees with \eqref{eq:timeshiftertransferfunction}.  % Finally consider the discrete-time system $H$ with impulse response 
% \[
...
\end{equation}
The spectrum is periodic with period $F$ and is plotted by the shaded region in Figure~\ref{fig:bandlimiteddiscretetimefiltering} in the case that $c = 1$ and $F = \tfrac{1}{P} = 4$.  When $F > 2c$, the system $L_c^P$ is called an \term{ideal lowpass digital filter} with cuttoff frequency $c$ and sample period $P$. Now consider the response of $L_c^P$ to the signal $x$ from~\eqref{eq:examplebandlimtedsignalforfiltering}.  If $c \in (\tfrac{1}{2}, \tfrac{5}{4})$ and $F > \tfrac{7}{2}$ then the response $L_c^Px$ has Fourier transform
\[
\calF L_c^P x = \Lambda L_c^P \, \calF x = \hat{x}(f) \sum_{m \in \ints} \rect\left( \frac{f + Fm}{2c}\right) = \tfrac{4}{3} \rect(f)
\]
%TODO: Use \cdot notation or even \times for multiplication of signals.  The above in potentially confusing.
...
Ideal highpass and ideal bandpass digital filters can be constructed using $L_c^P$ in much the same way that ideal analogue filters were constructed using the ideal lowpass filter $L_c$ in Section~\ref{sec:analogue-filters}.  An \term{ideal high pass digital filter} with cuttoff frequency $c$ and period $P = \tfrac{1}{F} < \tfrac{1}{2c}$ is the system given by the linear combination 
\[
T_0 - L_c^{P}.
\]  
...
% \end{align*}
% This response is plotted in 
The Fourier transform of the response of $T_0 - L_1^{1/4}$ to input signal $x$ from~\ref{eq:examplebandlimtedsignalforfiltering} is plotted in Figure~\ref{fig:bandlimiteddiscretetimefiltering} (middle).  The two higher frequency cosine pulses are preserved and the rectangular pulse at the origin is removed.

An \term{ideal bandpass digital filter} with upper cuttoff frequency $u$, lower cuttoff frequency $\ell$, and period $P$ is the system given by the linear combination 
\[
L_u^P - L_\ell^P.
\]  
...
\begin{figure}[tp]
  ...
  \begin{tikzpicture}
    \begin{scope}[yscale=\sy,xscale=\sx]
      ...
      % draw hatched region for magnitude spectrum of the ideal low pass filter. 
%      \tikzstyle{idealowpass}=[fill,pattern=north west lines,draw=none]
%      \draw[idealowpass] (-4.3,0) -- (-\c,0) -- (-\c,1) -- (\c,1) -- (\c,0) -- (4.3,0) -- cycle;
      ...
      % -- (0.5,0) -- (1.25,0) -- (1.25,1) -- (1.75,1) -- (1.75,0) -- (4,0);
       ...
       \draw[thick] (-4.1,0) -- (-0.5,0) -- (-0.5,1.3333) -- (0.5,1.3333) -- (0.5,0) -- (4.1,0);
...
\vspace{0.2cm}
  \begin{tikzpicture}
    \begin{scope}[yscale=\sy,xscale=\sx]
      ...
      % draw light gray region for magnitude spectrum of the discrete-time system. 
%      \fill[fill=lightgray] (-4.3,0) -- (-4.3,1) -- (-4+\c,1) -- (-4+\c,0) -- (-\c,0) -- (-\c,1) -- (\c,1) -- (\c,0) -- (4-\c,0) -- (4-\c,1) -- (4.3,1) -- (4.3,0) -- cycle;
            \fill[fill=lightgray] (-2-\c,0) -- (-2-\c,1) -- (-2+\c,1) -- (-2+\c,0) -- (2-\c,0) -- (2-\c,1) -- (2+\c,1) -- (2+\c,0) -- cycle;
...
\def\ell{0.25}
  \begin{tikzpicture}
    \begin{scope}[yscale=\sy,xscale=\sx]
      ...
      % draw light gray region for magnitude spectrum of the discrete-time system. 
  %    \fill[fill=lightgray] (-4.3,0) -- (-4.3,1) -- (-4+\c,1) -- (-4+\c,0) -- (-\c,0) -- (-\c,1) -- (\c,1) -- (\c,0) -- (4-\c,0) -- (4-\c,1) -- (4.3,1) -- (4.3,0) -- cycle;
      \fill[fill=lightgray] (-2-\u,0) -- (-2-\u,1) -- (-2-\ell,1) -- (-2-\ell,0) -- (-\u,0) -- (-\u,1) -- (-\ell,1) -- (-\ell,0) -- (\ell,0) -- (\ell,1) -- (\u,1) -- (\u,0) -- (2+\ell,0) -- (2+\ell,1) -- (2+\u,1) -- (2+\u,0) -- cycle;
...
%      \draw[idealowpass] (-4.3,0) -- (-\c,0) -- (-\c,1) -- (\c,1) -- (\c,0) -- (4.3,0) -- cycle;
      ...
      % -- (0.5,0) -- (1.25,0) -- (1.25,1) -- (1.75,1) -- (1.75,0) -- (4,0);
       ...
       %draw filtered Fourier transform magnitude
       \draw[thick] (-4,0) -- (-1.75,0) 
       -- plot[smooth,domain=-1.75:-1.5,samples=25] (\x,{-cos(2*pi*\x r)}) 
       -- (-1.5,0) -- (-0.5,0) -- (-0.5,1.3333) -- (-0.25,1.3333) -- (-0.25,0) -- (0.25, 0) -- (0.25,1.3333) -- (0.5,1.3333) -- (0.5,0) -- (1.5,0)
       -- plot[smooth,domain=1.5:1.75,samples=25] (\x,{-cos(2*pi*\x r)})
       -- (1.75,0) -- (4,0);
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Fourier transform $\tfrac{4}{3}\rect(f) - \cos(2\pi f) \big( \rect(2f - 3) + \rect(2f + 3) \big)$ of a bandlimited signal with bandwidth $b \leq \tfrac{7}{4}$ (dashed). The solid lines shows the magnitude of the Fourier transform of the response to this signal to the ideal lowpass digital filter $L_1^{1/4}$ (top), the ideal highpass digital filter $T_0 - L_1^{1/4}$ (middle), and the ideal bandpass digital filter $L_{3/2}^{1/4} - L_{1/4}^{1/4}$.  The spectra of these filters are shown by the shaded regions. The spectra have period $P = 4$.  The periodicity is not of consequence because the signal is bandlimited with bandwidth $b \leq \tfrac{7}{4} < \tfrac{F}{2} = 2$.} \label{fig:bandlimiteddiscretetimefiltering}
\end{figure}


\section{Finite impulse response filters}\label{sec:finite-impulse-resp}

Ideal digital filters are not realisable in practice.  We now turn our attention to practical digital filters.  The first of these are called \term{finite impulse response filters} or \term{FIR filters} for short.  One feature preventing practical implementation of the ideal lowpass digital filter $L_c^P$ is that the discrete impulse response $h_n = 2cP\sin(2cPn)$ is not of finite support.  Computing the response $\sum_{m \in \ints} h_m T_{Pm} x(t)$ for any time $t \in \reals$ might require summing an infinite number of terms.  A pragmatic approach to alleviate this problem is to first approximate the ideal analogue lowpass filter $L_c$ by a regular system with impulse response
\[
2c w(t) \sinc(2ct)
\]
where $w(t)$ is a bounded signal called a \term{window function} or \term{apodization function}.  The window function $w(t)$ is zero outside an interval $(-W/2, W/2)$ where $W > 0$ is called the \term{window width}.  Because of the window function this impulse response has finite support and is absolutely integrable.  

Common window functions include the rectangular window $w(t) = \rect(t/W)$, the triangular (or \term{Bartlett}) window $w(t) = \bigtriangleup(2t/W)$ where 
\[
...
\]
the raised cosine (or \term{Hann}) window
\[
w(t)  = \frac{1}{2} \rect\left(\frac{t}{W}\right)\left( 1 + \cos\left(\frac{2\pi t}{W}\right) \right),
\]
and the \term{Blackman window}~\citep{BlackmanTukeywindows}
...
\end{equation}
where $a_0 = \tfrac{21}{50}$, $a_1 = \tfrac{1}{2}$, and $a_2 = \tfrac{2}{25}$.  These window functions and their Fourier transforms are plotted in Figure~\ref{fig:windowfunctions}.  The Fourier transform of the Blackman window decays to zero particularly rapidly as $\abs{f}$ increases (Exercise~\ref{exer:blackmawindowfouriertransform}).
...
This Fourier transform is plotted in Figure~\ref{fig:windowfunctionsasWincrease} for the rectangular and Blackman windows when the cuttoff $c = 1$ and the width $W=1,3,10$.  The Fourier transform approaches the spectrum of the ideal lowpass filter as the window width $W$ increases.

A \term{low pass finite impulse response filter} with period $P$, cuttoff frequency $c$, and window $w$ is a discrete-time system $L_c^{P,w}$ with period $P$ having impulse response $h_n = 2c P w(nP) \sinc(2c nP)$.  That is, $h$ is given by multiplying the windowed sinc function by $P$ and sampling with period $P$.  Because $w(t)$ is zero outside the interval $(-W/2, W/2)$, it follows that $h_n = 0$ when $n P \notin (-FW/2, FW/2)$ and so only a finite number of elements of $h$ are nonzero.  For this reason, the response of $L_c^{P,w}$ to any input signal $x$ can be computed by the finite sum
\begin{equation}\label{eq:firfilterfinitesum}
L_c^{P,w}x(t) = \sum_{m = -a}^a h_m T_{Pm}x(t) \qquad \text{where $a = \floor{FW/2}$}
\end{equation}
and $\floor{FW/2}$ denotes the greatest integer less than or equal to $FW/2$.  The number of terms to be summed is $2a + 1$.  This is typically called the number of filter \term{taps}.  The number of non zero terms in the impulse response is at most the number of taps.  

The spectrum of this filter is given by periodic summation of the Fourier transform $\hat{w}(f) * \rect(f/2c)$ of the windowed sinc function, that is,
\[
\Lambda L_c^{P,w}(f) = \sum_{m \in \ints} \big(\hat{w}(t) *  \rect(f/2c) \big)(Ff + Fm) = \calD h(Pf).
\]
The magnitude spectrum $\sabs{\Lambda L_c^{P,w}(f)}$ is shown by the shaded regions in Figure~\ref{fig:bandlimiteddiscretetimefilteringW3} for the rectangular and Blackman windows. 

Now consider filtering the signal $x$ from~\eqref{eq:examplebandlimtedsignalforfiltering} using a low pass finite impulse response filter $L_c^{P,w}$.  Figure~\ref{fig:bandlimiteddiscretetimefilteringW3} shows  the response in the case that the cuttoff frequency $c = 1$, the sample rate $F = \tfrac{1}{P} = 4$, and for the rectangular window with window width $W = 3 $ and $8$ and a Blackman window with width $W = 8$.  The filter using the Blackman window is particularly effective.  Its response is close to that of the ideal lowpass filter $L_1$ from Figure~\ref{fig:bandlimiteddiscretetimefiltering}.  The rectangular window suffers from ocillations that decay slowly as the window width $W$ increases. 
...
\begin{tikzpicture}
  \begin{scope}[scale=2]
    \draw[->] (-1.35,0) -- (1.35,0) node[above] {$t$};
%    \draw[->] (0,-0.7) -- (0,1.3) node[above] {$\rect(t)$};
    \draw[->] (0,-0.5) -- (0,1.3) node[above] {};
  ...
  \begin{scope}[xscale=2]
    \vtick{0.5} node[pos=0.5,below] {$\tfrac{W}{2}$};
    \vtick{-0.5} node[pos=0.5,below] {$-\tfrac{W}{2}$};
  \end{scope}
\end{tikzpicture} 
\;
  \begin{tikzpicture}[domain=-4.8:4.8,samples=100]
    \begin{scope}[yscale=2,xscale=0.5]
      \draw[->] (-5.4,0) -- (5.4,0) node[above] {$f$};
%      \draw[->] (0,-0.7) -- (0,1.3) node[above] {$\sinc(t)$};
    \draw[->] (0,-0.5) -- (0,1.3) node[above] {};
      \draw[smooth,color=black,thick] plot function{\sincf(x)};
    \end{scope}
    \begin{scope}[yscale=2]
      \htick{1} node[pos=0.5,above left] {$W$};
    \end{scope}
    \begin{scope}[xscale=0.5]
      \vtick{1.0} node[pos=0.5,above right] {$\tfrac{1}{W}$};
      \vtick{-1.0} node[pos=0.5,above left] {$-\tfrac{1}{W}$};
    \end{scope}
  \end{tikzpicture} 
\\
\vspace{0.5cm}
...
%    \draw[->] (0,-0.5) -- (0,1.3) node[above] {$\bigtriangleup(f)$};
  ...
  \end{scope}
  \vtick{1} node[pos=0.5,below] {$\tfrac{W}{2}$};
  \vtick{-1} node[pos=0.5,below] {$-\tfrac{W}{2}$};
\end{tikzpicture}
\;
\begin{tikzpicture}
...
%    \draw[->] (0,-0.5) -- (0,1.3) node[above] {$\sinc^2(t)$};
        \draw[->] (0,-0.5) -- (0,1.3) node[above] {};
    \draw[smooth,color=black,thick,domain=-4.8:4.8,samples=100] plot function{\sincf(x/2)*\sincf(x/2)};
  \end{scope}
\begin{scope}[yscale=2]
  \htick{1} node[pos=0.5,above left] {$\tfrac{W}{2}$};
\end{scope}
\begin{scope}[xscale=0.5]
  \vtick{2} node[pos=0.5,below] {$\tfrac{2}{W}$};
  \vtick{-2} node[pos=0.5,below] {$-\tfrac{2}{W}$};
\end{scope}
\end{tikzpicture}
\\
...
%    \draw[->] (0,-0.7) -- (0,2.4) node[above] {$\rect(f) + \rect(f)\cos( 2\pi f )$};
    \draw[->] (0,-0.35) -- (0,1.3) node[above] {};
    \draw[thick] (-1.2,0)--(-0.5,0);  
    \draw[thick] (0.5,0)--(1.2,0);
    \draw[smooth,color=black,thick,domain=-0.5:0.5,samples=50] plot function{0.5*(1 + cos(2*3.1415926*x))};
\end{scope}
    \htick{2} node[pos=0.5,above left] {$1$};
\begin{scope}[xscale=2]
    \vtick{0.5} node[pos=0.5,below] {$\tfrac{W}{2}$};
    \vtick{-0.5} node[pos=0.5,below] {-$\tfrac{W}{2}$}; 
\end{scope}
...
\begin{scope}[yscale=2,xscale=0.5]
    \draw[->] (-5.4,0) -- (5.4,0) node[above] {$f$};
%    \draw[->] (0,-0.35) -- (0,1.3) node[above,font=\footnotesize] {$\sinc(t) + \tfrac{1}{2}\sinc(t+1) + \tfrac{1}{2}\sinc(t-1)$};    
    \draw[->] (0,-0.35) -- (0,1.3) node[above,font=\footnotesize] {};    
  ...
  \end{scope}
  \begin{scope}[xscale=0.5]
    \vtick{-2.0} node[pos=0.5,below] {$-\tfrac{2}{W}$};
    \vtick{2} node[pos=0.5,below] {$\tfrac{2}{W}$};
\end{scope}
  \begin{scope}[yscale=2]
    \htick{1} node[pos=0.5,above left] {$\tfrac{W}{2}$};
...
\begin{tikzpicture}
  \begin{scope}[xscale=2,yscale=2]
   ...
   % \draw[->] (0,-0.7) -- (0,2.4) node[above] {$\rect(f) + \rect(f)\cos( 2\pi f )$};
    ...
    \draw[thick] (-1.2,0)--(-0.5,0);  
    \draw[thick] (0.5,0)--(1.2,0);
    \draw[smooth,color=black,thick,domain=-0.5:0.5,samples=50] plot function{21.0/50 + cos(2*3.1415926*x)/2 + 2*cos(4*3.1415926*x)/25};
\end{scope}
\begin{scope}[yscale=2]
\htick{1} node[pos=0.5,above left] {$1$};
\end{scope}
...
\end{tikzpicture} 
\;
%\def\blackmanF(#1){(0.84-0.09*(#1*#1))*\sincf(#1/2)/2/(1-(#1*#1)/4)/(1-(#1*#1))}
\begin{tikzpicture}
\begin{scope}[yscale=2,xscale=0.5]
    \draw[->] (-5.4,0) -- (5.4,0) node[above] {$f$};
%    \draw[->] (0,-0.35) -- (0,1.2) node[above,font=\footnotesize] {$\sinc(t) + \tfrac{1}{2}\sinc(t+1) + \tfrac{1}{2}\sinc(t-1)$};    
    \draw[->] (0,-0.35) -- (0,1.3) node[above,font=\footnotesize] {};    
\begin{scope}[xscale=2]
  \clip (-2.4,-0.1) rectangle (2.4,1.3);
  \draw[smooth,color=black,thick,line cap=round] plot[] file {data/windowandfir/blackman_W2.0.csv};
\end{scope}
  \end{scope}
  \begin{scope}[xscale=0.5]
    \vtick{-2.0} node[pos=0.5,below] {$-\tfrac{2}{W}$};
...
\begin{figure}[tp]
  ...
  \begin{tikzpicture}
    \begin{scope}[yscale=\sy,xscale=\sx]
      ...
      % draw light gray region for magnitude spectrum of the discrete-time system. 
      \draw[thick] plot[] file {data/windowandfir/rect_W1.0_c1.0_re.csv};
      \draw[thick] plot[] file {data/windowandfir/rect_W3.0_c1.0_re.csv};
      \draw[thick] plot[] file {data/windowandfir/rect_W10.0_c1.0_re.csv};
%      \draw[thick] plot[] file {data/windowandfir/rect_W2.0_c1.0.csv};

      \draw[->] (-0.4,0) -- (3.3,0) node[above] {$f$};
      % \draw[->] (0,-0.35) -- (0,2.3) node[above,font=\footnotesize] {$\rect(f) \big(1 + \cos( 2 \pi f )\big) + \rect{3f - 2} + \rect{3f + 2}$};    
      \draw[->] (0,-0.15) -- (0,1.3) node[above,font=\footnotesize] {};  
      \node[font=\footnotesize] at (0.9,1.15) {$10$};
      \node[font=\footnotesize] at (0.68,1.15) {$3$};
      \node[font=\footnotesize] at (0.3,1.15) {$1$};
    ...
    \end{scope}
    \begin{scope}[yscale=\sy]
      \htick{1} node[pos=0.5,below left] {$1$};
    \end{scope}
  \end{tikzpicture}
\\
\vspace{0.5cm}
...
%       % draw light gray region for magnitude spectrum of the discrete-time system. 
%       \draw[thick] plot[] file {data/windowandfir/hann_W1.0_c1.0_re.csv};
%       \draw[thick] plot[] file {data/windowandfir/hann_W3.0_c1.0_re.csv};
%       \draw[thick] plot[] file {data/windowandfir/hann_W10.0_c1.0_re.csv};
% %      \draw[thick] plot[] file {data/windowandfir/rect_W2.0_c1.0.csv};

%       \draw[->] (-0.4,0) -- (3.3,0) node[above] {$f$};
%       % \draw[->] (0,-0.35) -- (0,2.3) node[above,font=\footnotesize] {$\rect(f) \big(1 + \cos( 2 \pi f )\big) + \rect{3f - 2} + \rect{3f + 2}$};    
%       \draw[->] (0,-0.25) -- (0,1.3) node[above,font=\footnotesize] {};  
...
% \vspace{0.5cm}
  \begin{tikzpicture}
    \begin{scope}[yscale=\sy,xscale=\sx]
      ...
      % draw light gray region for magnitude spectrum of the discrete-time system. 
      \draw[thick] plot[] file {data/windowandfir/blackman_W1.0_c1.0_re.csv};
      \draw[thick] plot[] file {data/windowandfir/blackman_W3.0_c1.0_re.csv};
      \draw[thick] plot[] file {data/windowandfir/blackman_W10.0_c1.0_re.csv};
%      \draw[thick] plot[] file {data/windowandfir/rect_W2.0_c1.0.csv};

      \draw[->] (-0.4,0) -- (3.3,0) node[above] {$f$};
      % \draw[->] (0,-0.35) -- (0,2.3) node[above,font=\footnotesize] {$\rect(f) \big(1 + \cos( 2 \pi f )\big) + \rect{3f - 2} + \rect{3f + 2}$};    
      \draw[->] (0,-0.15) -- (0,1.15) node[above,font=\footnotesize] {};  
      \node[font=\footnotesize] at (0.9,1.02) {$10$};
      \node[font=\footnotesize] at (0.7,0.88) {$3$};
      \node[font=\footnotesize] at (0.3,0.73) {$1$};
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Fourier transform $\hat{w}(f) * \rect(f)$ of windowed sinc functions for window width $W=1,3$, and $10$.  Top: rectangular window $w(t) = \rect(t/W)$.  Bottom: Blackman window~\eqref{eq:blackmanwindow}. The Fourier transforms approach $\rect(f)$ as $W$ increases.  The rectangular window exhibits large oscillations even when $W$ is large.} \label{fig:windowfunctionsasWincrease}
\end{figure}
...
\begin{figure}[p]
  ...
  \begin{tikzpicture}
    \begin{scope}[yscale=\sy,xscale=\sx]
      ...
      % draw light gray region for magnitude spectrum of the discrete-time system. 
      %\fill[fill=lightgray] (-4.3,0) -- (-4.3,1) -- (-4+\c,1) -- (-4+\c,0) -- (-\c,0) -- (-\c,1) -- (\c,1) -- (\c,0) -- (4-\c,0) -- (4-\c,1) -- (4.3,1) -- (4.3,0) -- cycle;
      \fill[fill=lightgray] (-4.3,0) -- plot[] file {data/windowandfir/rect_periodised_W2.0_c1.0.csv} -- (4.3,0) --cycle;
      ...
      % draw unfiltered Fourier transform magnitude
       ...
       %draw filtered Fourier transform magnitude
       \draw[thick] (-4,0) -- (-1.75,0) 
       -- plot[] file {data/windowandfir/rect_periodised_W2.0_c1.0_left.csv}
       -- (-1.25,0) -- (-0.5,0)
       -- plot[] file {data/windowandfir/rect_periodised_W2.0_c1.0_middle.csv}
       -- (0.5,0) -- (1.25, 0)
       -- plot[] file {data/windowandfir/rect_periodised_W2.0_c1.0_right.csv}
       -- (1.75,0) -- (4,0);
...
\vspace{0.5cm}
  ...
  %     % draw light gray region for magnitude spectrum of the discrete-time system. 
  %     %\fill[fill=lightgray] (-4.3,0) -- (-4.3,1) -- (-4+\c,1) -- (-4+\c,0) -- (-\c,0) -- (-\c,1) -- (\c,1) -- (\c,0) -- (4-\c,0) -- (4-\c,1) -- (4.3,1) -- (4.3,0) -- cycle;
  %     \fill[fill=lightgray] (-4.3,0) -- plot[] file {data/windowandfir/blackman_periodised_W3.0_c1.0.csv} -- (4.3,0) --cycle;

  %     % draw axis
  %     \draw[->] (-4.3,0) -- (4.3,0) node[above] {$t$};
  %     % \draw[->] (0,-0.35) -- (0,2.3) node[above,font=\footnotesize] {$\rect(f) \big(1 + \cos( 2 \pi f )\big) + \rect{3f - 2} + \rect{3f + 2}$};    
  %     \draw[->] (0,-0.2) -- (0,1.8) node[above,font=\footnotesize] {};  
      
  %     % draw unfiltered Fourier transform magnitude
  %      \draw[thick,dashed] (-4,0) -- (-1.75,0) 
  %      -- plot[smooth,domain=-1.75:-1.25,samples=50] (\x,{-cos(2*pi*\x r)})
  %      -- (-1.25,0) -- (-0.5,0) -- (-0.5,1.5) -- (0.5,1.5) -- (0.5,0) -- (1.25, 0)
  %      -- plot[smooth,domain=1.25:1.75,samples=50] (\x,{-cos(2*pi*\x r)})
  %      -- (1.75,0) -- (4,0);
      
  %      %draw filtered Fourier transform magnitude
  %      \draw[thick] (-4,0) -- (-1.75,0) 
  %      -- plot[] file {data/windowandfir/blackman_periodised_W3.0_c1.0_left.csv}
  %      -- (-1.25,0) -- (-0.5,0)
  %      -- plot[] file {data/windowandfir/blackman_periodised_W3.0_c1.0_middle.csv}
  %      -- (0.5,0) -- (1.25, 0)
  %      -- plot[] file {data/windowandfir/blackman_periodised_W3.0_c1.0_right.csv}
  %      -- (1.75,0) -- (4,0);
  ...
  \begin{tikzpicture}
    \begin{scope}[yscale=\sy,xscale=\sx]
      ...
      % draw light gray region for magnitude spectrum of the discrete-time system. 
      %\fill[fill=lightgray] (-4.3,0) -- (-4.3,1) -- (-4+\c,1) -- (-4+\c,0) -- (-\c,0) -- (-\c,1) -- (\c,1) -- (\c,0) -- (4-\c,0) -- (4-\c,1) -- (4.3,1) -- (4.3,0) -- cycle;
      \fill[fill=lightgray] (-4.3,0) -- plot[] file {data/windowandfir/rect_periodised_W8.0_c1.0.csv} -- (4.3,0) --cycle;
      ...
      % draw unfiltered Fourier transform magnitude
       ...
       %draw filtered Fourier transform magnitude
       \draw[thick] (-4,0) -- (-1.75,0) 
       -- plot[] file {data/windowandfir/rect_periodised_W8.0_c1.0_left.csv}
       -- (-1.25,0) -- (-0.5,0)
       -- plot[] file {data/windowandfir/rect_periodised_W8.0_c1.0_middle.csv}
       -- (0.5,0) -- (1.25, 0)
       -- plot[] file {data/windowandfir/rect_periodised_W8.0_c1.0_right.csv}
       -- (1.75,0) -- (4,0);
...
\vspace{0.5cm}
  \begin{tikzpicture}
    \begin{scope}[yscale=\sy,xscale=\sx]
      ...
      % draw light gray region for magnitude spectrum of the discrete-time system. 
      %\fill[fill=lightgray] (-4.3,0) -- (-4.3,1) -- (-4+\c,1) -- (-4+\c,0) -- (-\c,0) -- (-\c,1) -- (\c,1) -- (\c,0) -- (4-\c,0) -- (4-\c,1) -- (4.3,1) -- (4.3,0) -- cycle;
      \fill[fill=lightgray] (-4.3,0) -- plot[] file {data/windowandfir/blackman_periodised_W8.0_c1.0.csv} -- (4.3,0) --cycle;
      ...
      % draw unfiltered Fourier transform magnitude
       ...
       %draw filtered Fourier transform magnitude
       \draw[thick] (-4,0) -- (-1.75,0) 
       -- plot[] file {data/windowandfir/blackman_periodised_W8.0_c1.0_left.csv}
       -- (-1.25,0) -- (-0.5,0)
       -- plot[] file {data/windowandfir/blackman_periodised_W8.0_c1.0_middle.csv}
       -- (0.5,0) -- (1.25, 0)
       -- plot[] file {data/windowandfir/blackman_periodised_W8.0_c1.0_right.csv}
       -- (1.75,0) -- (4,0);
...
%     \end{scope}
%   \end{tikzpicture}
%   \caption{Window width $W = 8$.} \label{fig:bandlimiteddiscretetimefilteringW8}
% \end{figure}

\begin{test}\label{test:firfilteredlecturerec}
(\textbf{Finite impulse response filtered lecture recording})

We consider again the \SI{20}{\second} audio signal from Test~\ref{test:ftlecturerec}.  The mono audio recording contains $N = 440998$ samples denoted by $c_0,c_1,\dots,c_{N-1}$ at sample rate $F = \SI{22050}{\hertz}$.  We put $c_n = 0$ when $n < 0$ or $n \geq N$.  Let $x$ be the bandlimited signal with samples $c_n = x(nP)$, that is,
\[
x(t) = \sum_{n \in \ints} c_n \sinc(Ft - n).
\]  
In Test~\ref{test:ftlecturerec} the Fourier transform of $x$ was computed and plotted in Figure~\ref{fig:test:fouriertransformlecturevideo}.  The audio recording contains human voice primarily residing at lower frequencies below \SI{4}{\kilo\hertz}.  Audible in the recording is a faint high pitched hum.  This hum is represented in Figure~\ref{fig:test:fouriertransformlecturevideo} by spikes occurring at approximately $\pm\SI{8}{\kilo\hertz}$ and also by the region between \SI{4900}{\hertz} and \SI{5900}{\hertz} where the magnitude of the Fourier transform is elevated.  In this test, a low pass digital finite impulse response filter $L_{\gamma}^{P,w}$ will be used to remove this hum.

We choose the period of the filter to be $P = \tfrac{1}{F}$ where $F = \SI{22050}{\hertz}$ is the sample rate of the audio recording.  The cuttoff frequency is chosen to be $\gamma = \SI{4600}{\hertz}$ and a Blackman window~\eqref{eq:blackmanwindow} of width $W=\tfrac{12}{\gamma} = \tfrac{3}{1150}$ is used.  This filter has impulse response
\[
...
\[
a = \floor{\frac{FW}{2}} = \floor{\frac{1323}{46}} = 28.
\] 
This filter has $2a+1 = 57$ taps and can be implemented by a finite sum of 57 terms~\eqref{eq:firfilterfinitesum}.  The spectrum of the filter is
\[
\Lambda L_{\gamma}^{P,w}(f) = \calD h(Pf) = \sum_{n = -a}^a h_n e^{-2\pi j P f n}. 
\]
The magnitude spectrum $\sabs{\Lambda L_{\gamma}^{P,w}}$ is plotted in Figure~\ref{fig:test:magspecfirlowpass}.
% \[
% L_\gamma^{P,w}x(t) = \sum_{m \in \ints} h_m T_{Pm}x(t) = \sum_{m = -a}^a h_m T_{Pm}x(t) .
% \]

Let $y = L_\gamma^{P,w} x$ be the response of the filter to the bandlimited audio signal $x$.  We will only have need of the samples of the response $d_n = y(nP) = L_\gamma^{P,w} x(nP)$.  From~\eqref{eq:disctimesyssamplesonly} the sequence $d$ of samples is the discrete convolution of the impulse response $h$ and the sequence $c$ of audio samples, that is, 
\[
d_n = (h*c)_n = \sum_{m \in \ints} h_m c_{n-m} = \sum_{m = -a}^a h_m c_{n-m}.
\]
Observe that computing $d_n$ for any $n$ involves only a finite sum of $2a+1 = 57$ terms because $h_n = 0$ for $n > a$ or $n < -a$.  The samples $d_0, d_1, \dots, d_{N-1}$ are written to the file \texttt{nohum.wav}.  Listening to this file confirms that the high pitched hum is no longer audible.  

The response $y$ is the bandlimited signal with samples $d_n = L_\gamma^{P,w}$, that is, 
\[
y(t) = \sum_{n \in \ints} d_n \sinc(Ft - n).
\]  
The Fourier transform of $y$ is given by $\calF y(f) = P \rect(Pf) \calD d(Pf)$ where $\calD d$ is the discrete Fourier transform of the sequence $d$~\eqref{eq:disctransintro}.  The magnitude of the Fourier transform is plotted in Figure~\ref{fig:test:firfilteredlecturevideo}.  The spikes at $\pm\SI{8}{\kilo\hertz}$ and the elevated region between \SI{4900}{\hertz} and \SI{5900}{\hertz} no longer exist.
...
\begin{shaded}
    ...
    \begin{axis}[font=\footnotesize,ymode=log,height=8cm,width=12cm,xlabel={frequency},ylabel={$\abs{L_{\gamma}^{P,w}(f)}$},xmin=0,xmax=8000]
	\addplot[mark= ] table {tests/firlecturevideo/magnitude.csv};
     %   \addplot[mark= ,dashed] coordinates {(0,0.1) (8000,0.1)};
     %  \addplot[mark= ,dashed] coordinates {(0,1e-3) (8000,1e-3)};
   \end{axis}  
  \end{tikzpicture}   
\captionsetup{type=figure} 
\captionof{figure}{Magnitude spectrum of  the low pass finite impulse response digital filter $L_{\gamma}^{P,w}$ with cuttoff frequency $\gamma = \SI{4600}{\hertz}$, sample rate $F = \tfrac{1}{P} = \SI{22050}{\hertz}$, and Blackman window of width $W = \tfrac{12}{\gamma} = \tfrac{1}{400}$.  The vertical axis uses a logarithmic scale.}\label{fig:test:magspecfirlowpass}
...
\vspace{0.5cm}
  ...
  \begin{tikzpicture}
    ...
    \begin{axis}[compat=newest,font=\small,height=8cm,width=12cm,ylabel={$\abs{\calF \tilde{y}}$},xlabel={$f\;\;(\si{\hertz})$},legend style={draw=none},xmin=-11800,xmax=11800]
      \addplot+[mark=none,color=black,mark options={solid,fill=black,scale=0.7}] table[x index=0, y index=1] {tests/firlecturevideo/ft.csv};
    \end{axis}
  \end{tikzpicture}
\captionsetup{type=figure} 
\captionof{figure}{Plot of the magnitude of the Fourier transform $\calF y$ of the filtered audio signal $y = L_\gamma^{P,w} x$.  The plot looks similar to Figure~\ref{fig:test:fouriertransformlecturevideo} except that the spikes at $\pm\SI{8}{\kilo\hertz}$ and the elevated region between \SI{4900}{\hertz} and \SI{5900}{\hertz} no longer exist.}\label{fig:test:firfilteredlecturevideo}
\end{shaded}
\end{randomfloat}
