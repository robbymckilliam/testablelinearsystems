...
\begin{figure}[p]
  ...
  \begin{tikzpicture}[scale=\zscale]
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Pole zero plots for the discrete time systems corresponding with the first order difference equations $c_n = d_n - \tfrac{2}{3} d_{n-1}$ (top left) and $c_n = d_n + \tfrac{3}{2} d_{n-1}$ (top right) and the second order difference equations $z^2 - \tfrac{4}{3} x - \tfrac{4}{9}$ (bottom right) and $z^2 + \sqrt{2}z + 1$ (bottom right).  The plots of the left correspond with stable systems because all poles are contained inside the complex unit circle (dashed).  Plots on the left correspond with unstable systems because there exists poles on or outside the complex unit circle. }\label{fig:polezeroplotdisctime}
\end{figure}
}
