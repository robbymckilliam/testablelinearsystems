...
Let $H$ be a discrete time system with discrete impulse response $h$ and domain $\dom_P h$.  The response of $H$ to $e^{st} \in \dom_P h$ satifies
\begin{align*}
...
\]
This is precisely those $s \in \complex$ with strictly positive real part.  The transfer function is
\[
...
The spectrum is related to the discrete time Fourier transform of $h$ by
\[
...
As an example, suppse that $H$ is the system with period $P$ and discrete impulse response
\[
...
This conclusion needs to be taken with some degree of scepticism because the discrete impulse response $h_n = \sinc(n/4)$ is not absolutely summable and so the domain $\cep \dom_P h$ does not contain the imaginary axis.  Fortunately We will not have direct use of this system in what follows and so this caveat will not affect the results that follow.
...
\[
\calF(x) = \rect(f) - \cos(2\pi f) \big( \rect(2f - 3) + \tfrac{1}{2} \rect(2f + 3) \big)
\]
The Fourier transform is plotted is Figure~\ref{fig:bandlimiteddiscretetimefiltering} (dashed line).  The Fourier transform has a rectangular pulse centered at the origin and raised cosine pulses of width $\tfrac{1}{2}$ at $f = \pm \tfrac{3}{2}$.  The signal is bandlimited with bandwidth $\tfrac{7}{4}$.  Suppose that the component of interest is the rectangular pulse at the origin and that the two raised cosine pulses would preferably not exist.  A low pass filter could be used to filter out the rectangular pulses.  For example, an ideal lowpass filter (Section~\ref{}) with cuttoff frequency $c$ in the interval $(\tfrac{1}{2}, \tfrac{1}{2} )$ and spectrum $\rect(\tfrac{f}{2c})$ would preserve the recangular pulse at the origin and remove the two raised cosine pulses.  The impulse response of these ideal lowpass filter is $2c\sin(2ct)$.

The same effect can be achieved with a discrete time system. Because the signal has bandwidth $b = \tfrac{7}{4}$ the spectrum of this filter for frequencies $\abs{f} > \tfrac{7}{4}$ is not important.  Let $H$ be the discrete time system with discrete impulse response $h_n = 2cP \sinc(2c Pn)$ and period $P = \tfrac{1}{F}$ with $F > 2b = \frac{7}{2}$.  The discrete time Fourier transform of $h$ is given by~\eqref{eq:dftsignalperiodicsumP},
\[
\hat{h}(f) = \calD h(f) = \sum_{m \in \ints} \rect\left( \frac{Ff + Fm}{2c}\right)
\] 
and so the spectrum of $H$ is
\[
\Lambda H(f) = \hat{h}(Pf) = \sum_{m \in \ints} \rect\left( \frac{f + Fm}{2c}\right).
\]
The spectrum is periodic with period $F > 2b = \tfrac{7}{2}$.  Observe that A discrete time system configured for this purpose is called a low pass~\term{digital filter}.
