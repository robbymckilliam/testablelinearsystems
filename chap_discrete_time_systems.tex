...
\]
that is plotted by the dashed line in Figure~\ref{fig:bandlimiteddiscretetimefiltering}.  The Fourier transform has a rectangular pulse of height $\tfrac{4}{3}$ centered at the origin and raised cosine pulses of width $\tfrac{1}{2}$ at $f = \pm \tfrac{3}{2}$.  The signal is bandlimited with bandwidth $\tfrac{7}{4}$.  Suppose that the component of interest is the rectangular pulse at the origin and that the two raised cosine pulses would preferably not exist.  A low pass filter could be used to filter out the rectangular pulses.  For example, an ideal lowpass filter $L_c$ (Section~\ref{sec:analogue-filters}) with cuttoff frequency $c$ in the interval $(\tfrac{1}{2}, \tfrac{5}{4})$ and spectrum $\rect(\tfrac{f}{2c})$ would preserve the recangular pulse at the origin and remove the two raised cosine pulses.  The response of $x$ to $L_c$ has Fourier transform~\eqref{eq:fouriertransoutputsig}
\[
...
\end{equation}
The spectrum is periodic with period $F > 2b = \tfrac{7}{2}$ and is plotted by the shaded region in Figure~\ref{fig:bandlimiteddiscretetimefiltering} in the case that $c = 1$ and $F = 4$.  The system $H$ is called an \term{ideal lowpass digital filter} with cuttoff frequency $c$ and sample period $P$.  The response of $H$ to input signal $x$ has Fourier transform
\[
\calF H x = \Lambda H \, \calF x = \hat{x}(f) \sum_{m \in \ints} \rect\left( \frac{f + Fm}{2c}\right) = \tfrac{3}{2} \rect(f)
\]
exactly as with the ideal lowpass filter $L_c$.  This is seen in Figure~\ref{fig:bandlimiteddiscretetimefiltering} where only the period at the origin of the spectrum $\Lambda H$ overlaps with the Fourier transform of the bandlimited signal $x$.
...
\end{equation}
where $a_0 = \tfrac{21}{50}$, $a_1 = \tfrac{1}{2}$, and $a_2 = \tfrac{2}{25}$.  These window functions and their Fourier transform are plotted in Figure~\ref{fig:windowfunctions}.  The Fourier transform of the Blackman window decays to zero particularly rapidly as $\abs{f}$ increases.

From the multiplication property of the Fourier transform~\eqref{eq:multiplicationtheoremfouriertransform} we find the Fourier transform of the windowed sinc function $2c w(t) \sinc(2ct)$ to be the convolution of the Fourier transform of the window function $\calF w = \hat{w}$ and the Fourier transform $\calF\big( 2c\sinc(2ct) \big) = \rect(f/2c)$, that is,  
\[
\calF\big( 2c w(t) \sinc(2ct)  \big) = \hat{w}(f) * \rect(f/2c).
\]
These Fourier transforms for the rectangular and Blackman windows when the cuttoff $c = 1$ are plotted in Figure~\ref{?}.  Observe that the Fourier transform approaches the spectrum of the ideal lowpass filter as the window width $W$ increases.

A low pass finite impulse response filter with period $P$, cuttoff frequency $c$, and window $w$ is a discrete time system $H$ with period $P$ having impulse response $h_n = 2c w(nP) \sinc(2c nP)$ given by sampling the windowed sinc function.  Becasuse $w(t)$ is zero outside the interval $(-W/2, W/2)$ the impulse response $h_n = 0$ when $n P \notin (-FW/2, FW/2)$.  Only a finite number of elements of the impulse response $h$ are nonzero and so the response of $H$ to input signal $x$ can be computed by the finite sum
\[
Hx(t) = \sum_{m = -a}^a h_n T_{Pm}x(t) \qquad \text{where $a = \floor{FW/2}$}
\]
...
\[
\Lambda H(f) = \sum_{m \in \ints} \big(\hat{w}(t) *  \rect(f/2c) \big)(Ff + Fm)
\]
...
\end{scope}
\begin{scope}[xscale=0.5]
  \vtick{1} node[pos=0.5,below] {$\tfrac{2}{W}$};
  \vtick{-1} node[pos=0.5,below] {$-\tfrac{2}{W}$};
\end{scope}
\end{tikzpicture}
\\
...
\vspace{1cm}
  \begin{tikzpicture}
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Window width $W = 3$.} \label{fig:bandlimiteddiscretetimefiltering}
\end{figure}
...
\vspace{1cm}
  \begin{tikzpicture}
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Window width $W = 8$.} \label{fig:bandlimiteddiscretetimefiltering}
\end{figure}
