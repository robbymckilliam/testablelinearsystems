...
We now consider linear shift-invariant systems constructed from linear combinations of time-shifters of the form $T_{Pm}$ where $m \in \ints$ and $P$ is a positive real number called the~\term{sample period} or simply \term{period}.  That is, we consider systems of the form
\begin{equation}\label{eq:discretimesysdefn}
H x = \sum_{m \in \ints} h_m T_{ P m} x
\end{equation}
where $h \in \ints \to \complex$ is complex valued sequence.  Such systems are called \term{discrete time systems}.  It is common in the literature that the term discrete time system refers to a mapping from a sequence $x$ to another sequence $y$ of the form
\[
y_n = \sum_{m \in \ints} h_m x_{n-m}.
\]
We can recover this definition from~\eqref{eq:discretimesysdefn} by setting the period $P = 1$, sampling at the integers, and identifying $y_n$ with $Hx(n)$ and $x_n$ with $x(n)$.  %In this way,~\eqref{eq:discretimesysdefn} provides a more general setting from which to study discrete time systems.  
We will find that~\eqref{eq:discretimesysdefn} provides a setting in which to study discrete-time systems that closely links with the previous chapters where (what are often called continuous-time) systems have been studied.  Specifically, the complex exponetial signals $e^{st}$ remain the eigenfunctions and the notions of the transfer function and spectrum carry through unchanged.  This notion of a discrete-time system~\eqref{eq:discretimesysdefn} is connected with what \citet[Sec.~9.5]{Zemanian_dist_theory_1965} calls the continuous variable case of a linear \term{difference equation} with constant coefficients.  We will study difference equations further in Section~\ref{sec:difference-equations}.
...
\]
is not finite for any $t \in \reals$.  To define a suitable domain we will reuse the notation $\dom$ that was used to describe the domain of regular systems.  Given a sequence $h$ denote by $\dom h$ the set of signals $x$ such that
\[
\sum_{m \in \ints} \abs{h_m x(t - m)} < \infty \qquad \text{for all $t \in \reals$}.
\]
If $H$ has discrete impulse response $h$ then, for all signals $x \in \dom h$ we have
\[
\abs{Hx(t)} = \abs{\sum_{m \in \ints} h_m T_{ P m} x(t)} \leq \sum_{m \in \ints}  \abs{ h_m T_{ P m} x} < \infty
\]
for all $t \in \reals$ and so $Hx(t)$ is finite for all $t \in \reals$.  We take $\dom h$ as the domain of the discrete time system with discrete impulse response $h$ unless otherwise stated.  Be aware that set $\dom h$ is now defined for both signals $h$ and sequences $h$.  This reuse of notation is often called \term{overloading} in reference to the same concept used in many programming languages.

The discrete impulse response $h$ immediately yields some properties of the corresponding discrete time system $H$.  For example, if $h_m = 0$ for all $m < 0$, then $H$ is causal because 
\[
...
Let $F$ and $G$ be discrete time systems with equal sample period $P$ and discrete impulse responses $f$ and $g$.  Let $a,b\in\complex$ and let $H = a F + b G$ be the system formed by a linear combination of $F$ and $G$.  The response of $H$ to input signal $x \in \dom f \cap \dom g$ is
\begin{align*}
Hx &= a \sum_{n \in \ints} f_{n} T_{ P n}x + b \sum_{n \in \ints} g_{n} T_{ P n}x \\
&=  \sum_{n \in \ints} (a f_{n} + b g_{n}) T_{ P n}x,
\end{align*}
and so $H$ is a discrete time system with domain $\dom f \cap \dom g$ and discrete impulse response given by the linear combination of sequences $af + ag$.

Now suppose that $Hx = F Gx$ is formed by the composition of discrete time systems $F$ and $G$.  Denote by $\dom f g$ the set of signals $x$ such that
\[
\sum_{m \in \ints} \sum_{k \in \ints} \abs{f_m g_k x(t - m - k)} < \infty.
\]  
We will find $\dom f g$ to be a convenient domain for the system $H = FG$.  The response of $H$ to $x \in \dom f g$ is %BLERG THIS REQUIRES THE COMPOSITION TO EXIST, again defining domains appropriately is needed!
\begin{align*}
...
\end{align*}
Since $x \in \dom f\;g$ Fubini's theorem can be used to justify swapping the order of summation so that
\[
...
\]
This is called the \term{discrete convolution} of sequences $f$ and $g$.  The special notation $f * g$ is again used to denote the discrete convolution so that $h = f * g$.  The system $H$ constructed by composition of the discrete time systems $F$ and $G$ is a discrete time system.  The discrete impulse response of $H$ is the discrete convolution of the discrete impulse responses of $F$ and $G$.  The domain of $H$ is taken to be $\dom f\;g$ unless otherwise stated.  In what follows we will often use the term \term{convolution} and \term{impulse response} rather than the lengthier terms \term{discrete convolution} and \term{discrete impulse response} whenever there is no chance for confusion.
...
Recall that complex exponential signals are eigenfunctions of linear eigenfunctions of linear shift-invariant systems, that is, if $H$ is the linear-shift-invariant system with domain $X$, then the response of $H$ to input signal $e^{st} \in X$ satisfies
 \[
He^{st} = \lambda H(s) e^{st}
\]
where $\lambda H(s)$ is a complex number that depends on $s \in \complex$, but not on $t \in \reals$.  Considered as a function of $s$, the expression $\lambda H$ is called the transfer function of the system $H$.  The domain of $\lambda H$ is the set of complex numbers $s$ such that $e^{st} \in X$ and this set is denoted by $\cep X$.

Let $H$ be a discrete time system with discrete impulse response $h$ and domain $\dom h$.  The response of $H$ to $e^{st} \in dom h$ satifies
\begin{align*}
...
\[
\lambda H(s) = \sum_{n \in \ints} h_m e^{-sPn} \qquad s \in \cep \dom h.
\]
Putting $z = e^{Ps}$ we have
\[
\lambda(H, \tfrac{1}{P} \log z) = \sum_{n \in \ints} h_n z^{-n}
\]
that holds for complex numbers $z = e^{sP}$ such that $\frac{1}{P} \log z = s \in \cep\dom h$.  Denote this set by $\roc_\calZ h$.  The right hand side of the above expression is called the \term{z-transform} of the sequence $h$ and the set $\roc_\calZ h$ is called the region of convergence of the z-transform.

Let $c$ be a complex valued sequence.  The z-transform of $c$ is denoted by $\calZ c$.  It is the complex valued function of $\roc_\calZ c$ satisfying
\[
\calZ c(z) = \sum_{n \in \ints} c_n z^{-n} \qquad z \in \roc_\calZ c.
\]
The domain of $\calZ c$ is the region of convergence $\roc_\calZ c$.  It happens that the region of convergence $\roc_\calZ c$ is precisely the set of complex numbers $z$ such that the sequence $c_n z^{-n}$ is absolutely summable, that is, such that
\[
\sum_{n \in \ints} \abs{c_n z^{-n}} < \infty \qquad \text{if and only if $z \in \roc_\calZ c$}.
\]
The z-transform of a complex valued sequence plays a role analogous to the Laplace transform of a signal.  Recall that the region of convergence of the Laplace transform was either a half plane, a vertical strip, the entire complex plane, or the empty set (Section~\ref{sec:laplace-transform}).  We will find that the region of convergence of the z-transform is either a circular disk, an annular region in the complex plane, the complex plane with a disc at the origin removed, the entire complex plane, or the empty set.

We now consider some example z-transforms.  Consider first the step sequence $u$.  The region of convergence $\roc_\calZ u$ is all those complex numbers with magnitude greater than one because
\[
\sum_{n\in\ints} \abs{u_n z^{-n}} = \sum_{n=0}^\infty \abs{z}^{-n} < \infty
\]
only if $\abs{z} > 1$.  Graphically this region of convergence is the complex plane with a disc of radius one centered at the origin removed (Figure~\ref{fig:rocexampleztrans1}).  The z-transform of $u$ is
\[
\calZ u(z) = \sum_{n \in \ints} u_n z^{-n} = \sum_{n=0}^\infty z^{-n} = \frac{z}{z - 1} \qquad \abs{z} > 1. \qquad \text{(Exercise~\ref{exer:stepseqZtrans})}
\]

Now consider the sequence with $n$th element $(\tfrac{1}{2})^{n} u_n$.  The region of convergence is all those complex numbers with magnitude greater than $\tfrac{1}{2}$.   The z-transform
\[
...
\end{cases}
\]
In this case the region of convergence is all those complex numbers with magnitude less than $\tfrac{3}{2}$ because
\[
\sum_{n\in\ints} \abs{ (\tfrac{3}{2})^{n} u_{-n} z^{-n}} = \sum_{n=0}^\infty \abs{\tfrac{2}{3}z}^{n} < \infty
\]
only if $\abs{z} < \tfrac{3}{2}$.  The z-transform is
\[
...
\]
The region of convergence is an open disc of radius $\frac{3}{2}$ centered at the origin of the complex plane.  The sequence with $n$th element $(\tfrac{1}{2})^{n} u_n + (\frac{3}{2})^{n} u_{-n}$ has z-transform
\[
\calZ\big((\tfrac{1}{2})^{n} u_n + (\tfrac{3}{2})^{n} u_{-n}\big) =  \frac{2z}{2z - 1} + \frac{3}{3 - 2z} \qquad \tfrac{1}{2} < \abs{z} < \tfrac{3}{2}
\]
with region of convergence all those complex numbers such that $\tfrac{1}{2} < \abs{z} < \tfrac{3}{2}$.  The region of convergence is an annulus in the complex plane with inner radius $\tfrac{1}{2}$ and outer radius $\tfrac{3}{2}$.
...
\]
The region of convergence is the entire complex plane, that is, $\roc_\calZ \delta = \complex$.  Finally, consider the sequence $\onebf$ that has every element equal to $1$.  Because $\sum_{n \in \ints} \abs{z}^{-n}$ does not converge for any $z \in \complex$ the region of convergence $\roc_\calZ \onebf$ is the empty set.  The sequence $\onebf$ is said not to have a z-transform.
...
\]
Let $g$ be a sequence with region of convergence $R_g$.  The z-transform of the shifted sequence with $n$th element $g_{n-\ell}$ where $\ell \in \ints$ is related to that of $g$ by
\begin{align}
\calZ(g_{n-\ell}) &= \sum_{n \in \ints} g_{n-\ell} z^{-n} \nonumber \\
&= \sum_{n \in \ints} g_{n} z^{-(n+\ell)} \nonumber \\
...
\]
The spectrum is periodic with period equal to the reciprocal of the sample period $F = \frac{1}{P}$ called the \term{sample rate}. The spectrum is related to the discrete time Fourier transform of $h$ by
\[
