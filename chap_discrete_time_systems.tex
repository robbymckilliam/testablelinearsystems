...
Because the signal $x$ has bandwidth $b = \tfrac{7}{4}$ the spectrum of the filter for frequencies $\abs{f} > \tfrac{7}{4}$ is not important.  By exploting this property we will find that the same lowpass filtering can be achieved with a discrete time system.  Denote by $L_{c,P}$ the discrete time system with sample period $P = \tfrac{1}{F}$ and discrete impulse response $h_n = 2cP \sinc(2c Pn)$ given by sampling the impulse response of the ideal low pass filter $L_c$.  From~\eqref{eq:dftsignalperiodicsumP}, the discrete time Fourier transform of $h$ is given by periodic summation of $\rect(Ff/2c)$, that is,
\[
\hat{h}(f) = \calD h(f) = \sum_{m \in \ints} \rect\left( \frac{Ff + Fm}{2c}\right).
\] 
We are lead to believe that the spectrum of $L_{c,P}$ is
\begin{equation}\label{eq:LambdaHidealklowpassdigitalsceptical}
\Lambda L_{c,P}(f) = \hat{h}(Pf) = \sum_{m \in \ints} \rect\left( \frac{f + Fm}{2c}\right).
\end{equation}
The spectrum is periodic with period $F$ and is plotted by the shaded region in Figure~\ref{fig:bandlimiteddiscretetimefiltering} in the case that $c = 1$ and $F = 4$.  The system $L_{c,P}$ is called an \term{ideal lowpass digital filter} with cuttoff frequency $c$ and sample period $P$.  If $c \in (\tfrac{1}{2}, \tfrac{5}{4})$ and $F > \tfrac{7}{2}$ then the response of $L_{c,P}$ to input signal $x$ from~\eqref{eq:examplebandlimtedsignalforfiltering} has Fourier transform
\[
\calF L_{c,P} x = \Lambda H \, \calF x = \hat{x}(f) \sum_{m \in \ints} \rect\left( \frac{f + Fm}{2c}\right) = \tfrac{3}{2} \rect(f)
\]
exactly as with the ideal lowpass filter $L_c$.  This is seen in Figure~\ref{fig:bandlimiteddiscretetimefiltering} where only the period at the origin of the spectrum $\Lambda L_{1,0.25}$ overlaps with the Fourier transform of the bandlimited signal $x$.
...
%      \draw[idealowpass] (-4.3,0) -- (-\c,0) -- (-\c,1) -- (\c,1) -- (\c,0) -- (4.3,0) -- cycle;
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Fourier transform $\tfrac{3}{2}\rect(f) - \cos(2\pi f) \big( \rect(2f - 3) + \rect(2f + 3) \big)$ of a bandlimited signal with bandwidth $b \leq \tfrac{7}{4}$ (dashed). The solid line shows the magnitude of the Fourier transform of the response to this signal of the ideal lowpass digital filter with cuttoff $c=1$ and sample rate $F = \tfrac{1}{P} = 4$.  %The spectrum of this filter is shown by the hatched rectangular pulse at the origin.  
The higher frequency rasied cosine pulses centered at $f = \pm \tfrac{3}{2}$ are removed by the filter, but the rectangular pulse of height $\tfrac{3}{2}$ centered at the origin is unaffected.  The shaded region shows the periodic magnitude spectrum of this digital filter.  The period is $F = 4$. The periodicity is not of consequence because the signal is bandlimited with bandwidth $b \leq \tfrac{7}{4} < \tfrac{F}{2} = 2$.} \label{fig:bandlimiteddiscretetimefiltering}
\end{figure}
