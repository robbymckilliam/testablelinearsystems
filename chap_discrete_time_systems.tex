...
%      \draw[idealowpass] (-4.3,0) -- (-\c,0) -- (-\c,1) -- (\c,1) -- (\c,0) -- (4.3,0) -- cycle;
      ...
      % -- (0.5,0) -- (1.25,0) -- (1.25,1) -- (1.75,1) -- (1.75,0) -- (4,0);
       ...
       \draw[thick] (-4.1,0) -- (-0.5,0) -- (-0.5,1.5) -- (0.5,1.5) -- (0.5,0) -- (4.1,0);
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Fourier transform $\tfrac{3}{2}\rect(f) - \cos(2\pi f) \big( \rect(2f - 3) + \rect(2f + 3) \big)$ of a bandlimited signal with bandwidth $b < 2$ (dashed). The solid line shows the magnitude of the Fourier transform of the response to this signal of the ideal low pass filter with cuttoff frequency $c = 1$.  %The spectrum of this filter is shown by the hatched rectangular pulse at the origin.  
The higher frequency rasied cosine pulses centered at $f = \pm \tfrac{3}{2}$ are removed by the filter, but the rectangular pulse of height $\tfrac{3}{2}$ centered at the origin is unaffected.  The same affect is had by inputing this signal to an ideal lowpass digital filter with cuttoff $c=1$ and sample rate $F = \tfrac{1}{P} = 4$.  The shaded region shows the periodic magnitude spectrum of this digital filter.  The period is $F = 4$. The periodicity is not of consequence because the signal is bandlimited with bandwidth $b < \tfrac{F}{2} = 2$.} \label{fig:bandlimiteddiscretetimefiltering}
\end{figure}
