...
\end{cases}
\]
The function $f$ maps Mario to Conker, Link to Sephiroth, and Ness to Sephiroth.  The set $X$ is called the \term{domain} of the function $f$ and the set $Y$ a \term{range}.  The value of $f$ for input $x$ is denoted by $f(x)$ so, for example, $f(Mario) = Conker$ and $f(Link) = Sephiroth$.  The set of all functions mapping $X$ to $Y$ is denoted by $X \to Y$ and so $f \in X \to Y$ in the example above.

A \term{signal} is a function that maps each element from the set of real numbers $\reals$ to elements from the set of complex numbers $\complex$, that is, a function from the set $\reals \to \complex$.  For example
\[
\sin( \pi t), \qquad \frac{1}{2} t^3, \qquad e^{-t^2}
\]   
all represent signals with $t \in \reals$.  These signals are plotted in Figure~\ref{fig:signalsstart}.  Many physical phenomena, such as sound, light, weather, and motion, can be modelled using signals.  In this text we primarily focus on examples from electrical and mechanical engineering where signals are use to model changes in voltage, current, position, and angle over time.  In these examples, the independent variable $t$ will represent time.  However, there is no fundamental reason for this and the techniques developed here can be applied equally well when $t$ represents a quantity other than time.  An example where this occurs is image processing.
...
\end{cases}
\end{equation}
is right-sided.  Its horizontal reflection $u(-t)$ is left sided (Figure~\ref{fig:stepsided}).  A signal $x$ is said to have \term{finite support} if it is both left and right sided, that is, if there exits a $T\in\reals$ such that $x(t) = x(-t) = 0$ for all $t > T$.  The signals $\sin( \pi t)$ and $e^{-t^2}$ do note have finite support, but the \term{rectangular pulse}
...
A signal $x$ is \term{even} (or \term{symmetric}) if $x(t) = x(-t)$ for all $t \in \reals$ and \term{odd} (or \term{antisymmetric}) if $x(t) = -x(-t)$ for all $t \in \reals$.  For example, $\sin(\pi t)$ and $\tfrac{1}{2}t^3$ are odd and $e^{-t^2}$ is even.  % Every signal $x$ can be expressed as a sum $x = x_e + x_o$ where 
% \[
...
A signal $x$ is \term{conjugate symmetric} if $x(t) = x(-t)^*$ for all $t \in \reals$ $x$ and \term{conjugate antisymmetric} if $x(t) = -x(-t)^*$ for all $t \in \reals$, where $*$ denotes the complex conjugate of a complex number.  Equivalently, $x$ is conjugate symmetric if its real part $\Re(x)$ is an even signal and its imaginary part $\Im(x)$ is an odd signal and $x$ if conjugate antisymmetric if real part is odd and imaginary part even.  For example, the signal $e^{-t^2} + j \sin(\pi t)$ where $j = \sqrt{-1}$ is conjugate symmetric and the signal $\tfrac{1}{2}t^{3} + j e^{-t^2}$ is conjugate antisymmetric.

A signal $x$ is \term{continuous at} $t \in \reals$ if
\[
\lim_{h \to 0} x(t + h) = \lim_{h \to 0} x(t-h)
\]
and $x$ is said to be \term{continuous} if it is continuous at all real numbers $t$.  The signals $\sin( \pi t)$, $\frac{1}{2} t^3$, $e^{-t^2}$ and $t^2$ are all continuous, but the step function $u$ is not continuous at zero because
\[
\lim_{h \to 0} u(t + h) = 1 \neq 0 = \lim_{h \to 0} u(t-h).
\]
A signal $x$ is \term{continuously differentiable} or just \term{differentiable} if
\[
\lim_{h \to 0} \frac{x(t + h) - x(t)}{h} = \lim_{h \to 0} \frac{x(t) - x(t-h)}{h} \qquad \text{for all $t \in \reals$}.
\]
Considered as a function of $t$ the limit above is called the derivative of $x$ at $t$ and is typically denoted by $\frac{d}{dt} x(t)$.  For example, the signals $\sin( \pi t)$, $\frac{1}{2} t^3$, $e^{-t^2}$ and $t^2$ are differentiable with derivatives
\[
\pi \cos(\pi t), \qquad \tfrac{3}{2} t^{2}, \qquad -2t e^{-t^2}, \qquad 2t,
\] 
but the step function $u$ and the rectangular pulse $\rect$ are not differentiable (Exercise~\ref{exer:steprectnotdiff}).
...
\end{tikzpicture}
\caption{Derivatives of the signals $\sin( \pi t)$, $\frac{1}{2} t^3$, $e^{-t^2}$  from Figure~\ref{fig:signalsstart}.} \label{fig:signalsstart}
\end{figure}
...
\]
for all finite constants $a$ and $b$, where by $< \infty$ we mean that the integral evaluates to a finite number.  The signals $\sin(\pi t)$, $\tfrac{1}{2}t^3$, and $e^{-t^2}$ are all locally integrable.  An example of a signal that is not locally integrable is $x(t) = \frac{1}{t}$ (Exercise~\ref{exer:oneontnotlocallyint}).  
...
\]
The real number $\|x\|_2$ is called the \term{$L^2$-norm} of $x$.  Square integrable signals are also called \term{energy signals}, and the squared $L^2$-norm $\|x\|_2^2$ is also called the \term{energy} of $x$. For example $\sin( \pi t)$ and $\tfrac{1}{2}t^3$ are not energy signals, but $e^{-t^2}$ is.  It has energy $\|e^{-t^2}\|^2 = \sqrt{\pi}$ (Exercise~\ref{excer:energyexpchangevar}). The set of square integrable signals is often denoted by $L^2$ or $L^2(\reals)$.  %A discrete-time signal $x$ is an \term{energy signal} if it is \term{square summable}, that is, if
% \[
...
We denote systems with capital letters such as $H$ and $G$.  A system $H$ is a function that maps a signal $x$ to another signal denoted $H(x)$.  We call $x$ the \term{input signal} and $H(x)$ the \term{output signal} or the \term{response} of system $H$ to signal $x$.  We will often drop the brackets and write simply $Hx$ for the response of $H$ to $x$.  
\footnote{It is customary to drop the brackets only where $H$ is a \term{linear} system (Section~\ref{sec:properties-systems}).  We deal primarily with linear systems in this text and so dropping the brackets will rarely be a Faux Pas.}.  
The value of the output signal $Hx$ at $t$ is denoted by $Hx(t)$ or $H(x)(t)$ or $H(x,t)$ and we do not distinguish between these notations.   %~\citep{Schönfinkel_currying_1924,Currying_1968}.  
...
\begin{figure}[p]
  ...
  \end{tikzpicture}
  \caption{Shifter systems $T_{1.5}x(t) = x(t - 1.5)$ and $T_{-3}x(t) = x(t + 3)$ acting on the signal $x(t) = e^{-t^2}$.} \label{fig:timeshifter}
\end{figure}
...
\]
Figure~\ref{fig:timescaler} depicts the action of a time-scaler with a number of values for $\alpha$.  When $\alpha=-1$ the time-scaler reflects the input signal in the $t$ axis.  When $\alpha = 1$ the time-scaler is the identity system $T_0$.  Both the shifter and time-scaler are well defined for all signal and so it is reasonable to choose their domains as the entire set of signals $\reals \to \complex$.
...
\end{cases}
\]
Observe that the integrator $I_\infty$ cannot be applied to the signal $x(t) = t$ because $\int_{-\infty}^{t} \tau d\tau$ is not finite for any $t$.  A domain for $I_\infty$ would not contain the signal $x(t) = t$.  As described in Section~\ref{sec:syst-funct-sign} example of domains for $I_\infty$ are the set of absolutely integrable signals $L_1$ and the set of signals that are both right sided and locally integrable.


\section{Subsets of signals}\label{sec:spaces-signals}

The domain and range of a system are subsets of the set of signals $\reals \to \complex$. Before further describing the properties of systems we describe two important types of subsets.  These are the \term{linear spaces} and those subsets with the property of \term{shift-invariance}.  

Let $x$ and $y$ be signals.  We denote by $x + y$ the signal that takes the value $x(t) + y(t)$ for each $t  \in \reals$, that is, the signal that results from adding $x$ and $y$.  For $a$ a complex number we denote by $ax$ the signal that takes the value $a x(t)$ for each $t \in \reals$, that is, the signal that results from multiplying $x$ by $a$.  For signals $x$ and $y$ and complex numbers $a$ and $b$ the signal
\[
ax + by
\]
is called a \term{linear combination} of $x$ and $y$.  %Figure~\ref{fig:multiandaddofsignals} plots examples of 

Let $X$ be a set of signals.  The set $X$ is called a \term{linear space} (or \term{vector space}) if for all signals $x$ and $y$ from $X$ and all complex numbers $a$ and $b$ the signal formed by the linear combination $ax + by$ is also in $X$.  The set of all signals $\reals \to \complex$ is a linear space.  Another example is the set of differentiable signals, because, if $x$ and $y$ are differentiable, then the linear combination $ax + by$ is differentiable.  The derivative is $a \frac{d}{dt} x + b \frac{d}{dt} y$.  The set of even signals is another example of a linear space because if $x$ and $y$ are even then
\[
ax(t) + by(t) = a x(-t) + b y(-t)
\]
and so the linear combination $ax + by$ is even.  The set of absolutely integrable signals $L_1$ and the set of square integrable signals $L_2$ are examples of linear spaces (Exercise~\ref{exer:L1L2linearshiftinvariant}).  The set of periodic signals is an example that is not a linear space (Exercise~\ref{exer:periodicshiftinvariantnotlinear}).

A set of signals $X$ is \term{shift-invariant} if for all signals $x \in X$ and all real numbers $\tau$ the shifted signal $T_\tau x$ is also in $X$.  The set of differentiable signals is shift invariant because, if $x$ is differentiable, then the signal $T_\tau x(t) = x(t-\tau)$ is differentiable.  The set of periodic signals is shift invariant (Exercise~\ref{exer:periodicshiftinvariantnotlinear}) as are $L_1$ and $L_2$~\eqref{exer:L1L2linearshiftinvariant}.  The set of even signals and the set of odd signals are not shift invariant.
...
A system $H$ is called \term{memoryless} if the output signal $Hx$ at time $t$ depends only on the input signal $x$ at time $t$.  For example $\frac{1}{x(t)}$ and the identity system $T_0$ are memoryless, but 
\[
x(t) + 3 x(t-1) \qquad \text{and} \qquad \int_{0}^{1} x(t - \tau) d\tau
\] 
are not.  A shifter $T_\tau$ with $\tau \neq 0$ is not memoryless.

A system $H$ is \term{causal} if the output signal $Hx$ at time $t$ depends on the input signal only at times less than or equal to $t$.  Memoryless systems such as $\frac{1}{x(t)}$ and $T_0$ are also causal.  shifters $T_\tau$ are causal when $\tau \geq 0$, but are not causal when $\tau < 0$.  The systems 
\[
x(t) + 3 x(t-1) \qquad \text{and} \qquad \int_{0}^{1} x(t - \tau) d\tau
\] 
...
Let $H \in X \to Y$ with domain $X$ and range $Y$ being linear spaces of signals.  The system $H$ is \term{linear} if
\[
H( ax + by) = a Hx + b Hy
\]
...
\begin{tikzpicture}[node distance=1.8cm,auto,>=latex',yshift=-0.5]
    ...
    \node [dspnodeopen] (res) [right of=adder,label=above:$aHx+ bHy$] {};
    \draw[dspconn] (x) -- (xH);
...
Let $H \in X \to Y$ be a system with domain $X$ and range $Y$ being shift-invariant spaces of signals. A system $H$ is \term{shift-invariant} if
\[
H T_\tau x(t) = Hx(t-\tau)
\]
...
H T_\tau x = T_\tau Hx
\]
for all $\tau \in \reals$ and all signals $x \in X$.  Figure~\ref{blockdiag:timeinvariance} represents the property of shift-invariance with a block diagram.  % A consequence of time-invariance is
