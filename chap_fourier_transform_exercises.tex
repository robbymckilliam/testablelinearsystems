...
\item \label{exer:parsevalconsq} Given~\eqref{eq:parseval} show that~\eqref{eq:parsevalxy} holds.
\begin{solution}
\newcommand{\dotprod}[2]{\langle #1, #2 \rangle}
From~\eqref{eq:parseval} we have that
\[
...
\]
And since $\|x\|_2^2 = \|\hat{x}\|_2^2$ and $\|y\|_2^2 = \|\hat{y}\|_2^2$ as a result of~\eqref{eq:parseval} we find that
\[
\dotprod{x}{y} = \dotprod{\hat{x}}{\hat{y}}
\]
which is precisely~\eqref{eq:parsevalxy}.
\end{solution}

\reallyharditem \label{exer:ftdftswapsummationldct}Let $c$ be an absolutely summable sequence.  Show that 
\[
...
\reallyharditem \label{exer:ftdftswapsummationldct}Let $c$ be a square summable sequence and let  
\[
