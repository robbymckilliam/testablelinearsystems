...
\begin{solution}
The set $\fin h$ contains those signals $x$ for which
\[
\int_{-\infty}^\infty \abs{h(\tau)x(t - \tau)} d\tau < \infty \qquad \text{for all $t \in \reals$.}
\]
Suppose that $x, y \in \fin h$ and $a, b \in \complex$.  Then
\begin{align*}
...
\end{align*}
Both integrals on the right hand side are finite for all $t \in \reals$ becase $x$ and $y$ are in $\fin h$.  Thus the right hand side is finite for all $t\in\reals$ and so the linear combination $ax + by \in \fin h$.  It follows that $\fin h$ is a linear space.

If $x \in \fin h$ then
\[
\int_{-\infty}^\infty \abs{h(\tau) T_k x(t - \tau)} d\tau = \int_{-\infty}^\infty \abs{h(\tau) x(t - \tau - k)} d\tau < \infty
\]
for all $t \in \reals$ and so the shifted signal $T_k x \in \fin h$.  It follows that $\fin h$ is a shift-invariant space.
\end{solution}
...
\begin{solution}
Let $H$ be a regular system and $h$ its impulse response.  By default the domain of $H$ is supposed to be $\fin h$, that is $H \in \fin h \to \complex$.  If $h$ is absolutely integrable then for all signals $x \in \fin h$ such that $\sabs{x(t)} < M$ for all $t$,
\begin{align*}
\abs{H x(t)} &= \abs{(h * x)(t)} \\
...
\end{cases}
\]
Observe that $s$ is not in the domain $\fin h$ because
\[
...
\]
because $h$ is locally integrable (the impulse response is always locally integrable by assumption.  See Section~\ref{sec:conv-regul-syst}).  Put $M > 1$ and suppose that $H$ was stable.  Then there exists $K > 0$ such that $\abs{H x(t)} < K$ for all $t \in \reals$ and all $x \in \fin h$ bounded less that $M$.  Observe that $r_\kappa$ is bounded less than $M$, that is, $\abs{r_\kappa(t)} \leq 1 < M$ for all $\kappa \in \reals$ and all $t \in \reals$.  The response of $H$ to $r_\kappa$ at time zero is
\[
