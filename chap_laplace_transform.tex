...
The domain of the Laplace transform $\calL x$ is not always the entire complex plane $\complex$.  Let $\roc(x)$ be the set of complex numbers $s$ such that $x(t) e^{-s t}$ is absolutely integrable, that is,
\[
\int_{-\infty}^{\infty} \abs{x(t) e^{-s t}} dt < \infty  \qquad \text{if and only if $s \in \roc(x)$}.
\]
The set $\roc(x) \subseteq \complex$ is called the \term{region of convergence} of $\calL x$.  The integral~\eqref{eq:laplace_defn} is finite for all $s \in \roc(x)$ because 
\[
\sabs{\calL x(s)} = \abs{\int_{-\infty}^{\infty} x(t) e^{-st} dt} \leq \int_{-\infty}^{\infty} \abs{x(t) e^{-s t}} dt < \infty
\]
for all $s \in \roc(x)$.  We take $\roc(x)$ as the domain of $\calL x$ and so $\calL x \in \roc(x) \to \complex$.  That is, the Laplace transform of a signal is a complex valued function of the region of convergence.

For example, the Laplace transform of the right sided signal $e^{ \alpha t}u(t)$ has region of convergence given by all those complex number with real part greater than the real part of $\alpha$, that is,
\[
...
In the previous example the region of convergence is the empty set if $\Re(\alpha) \geq \Re(\beta)$.  The Laplace transform is said not to exist in this case.  Other signals have this property.  For example, the signal $x(t) = 1$ has no Laplace transform because
\[
...
\]
for all $s \in \complex$.  The Laplace transform of $\rect$,
...
\end{equation}
where $n \geq 0$ is an integer (Exercise~\ref{excer:laplacetransformcommonpolyut}).  The Laplace transforms of the signal $x(t)$ and the signal $e^{\alpha t}x(t)$ are related.  First observe that the region of convergence of $e^{\alpha t}x(t)$ is all those complex numbers $s$ such that $s - \alpha \in \roc(x)$.  Now,
\begin{align}
...
A useful relationship exists between the Laplace transform of a signal $x$ and its time-scaled version $x(\alpha t)$ where $\alpha \neq 0$. The region of convergence of the time-scaled signal $x(\alpha t)$ is those complex number $s$ such that $s/\alpha \in \roc(x)$.  The Laplace transform satisfies
...
Recall from Section~\ref{sec:eigenf-line-time} that complex exponential signals are \term{eigenfunctions} of linear shift-invariant systems.  That is, $H \in X \to Y$ is a linear shift-invariant system and if $e^{st} \in X$ then the response of $H$ to the signal $e^{st}$ satisfies $H e^{st} = \lambda H(s) e^{st}$.

Suppose that $H \in X \to Y$ is a regular system with impulse response $h$ and that the complex exponential signal $e^{st} \in X$ whenever $s \in R(h)$.  If $s \in \roc(h)$, then
\begin{align*}
...
Let $H_1 \in X \to Y$ and $H_2 \in X \to Y$ be linear shift-invariant systems with domain $X$ and range $Y$.  Let $H = a H_1 + bH_2$ be the system formed by a  linear combination of $H_1$ and $H_2$.  The response of $H$ to the complex exponential signal $e^{st} \in X$ is
\begin{align*}
...
Now let $H_1 \in X \to Y$ and $H_2 \in Y \to Z$ be linear shift-invariant systems and let $H \in X \to Z$ be the system formed by the composition $H = H_2 H_1$.  The response of $H$ to the signal $e^{st}$ is
\begin{align*}
H e^{st} &= H_2 H_1 e^{st} \\
...
We showed in Section~\ref{sec:line-comb-comp} that if $H_1$ and $H_2$ are regular systems with impulse responses $h_1$ and $h_2$, then the impulse of the composition $H = H_2 H_1$ is given by the convolution $h = h_1 * h_2$.  Because,
\[
...
\calL(x*y) = \calL x \calL y, \qquad s \in R(x) \cap R(y).
\end{equation}
...
Let $y = H x$ be the response of the system $H$ to input signal $x$.  In the case that $H$ is regular with impulse response $h$ we have $y = h * x$ and the convolution theorem asserts that
\begin{equation}\label{eq:transferLaplcetheorem}
\calL y = \calL h \calL x = \lambda H \calL x,
\end{equation}
Thus, the Laplace transform of the output signal $y = H x$ is the transfer function of the system $H$ multiplied by the Laplace transform of the input signal $x$.  This result also holds when $H$ is a shifter, that is,
\[
\calL T_{\tau} x = \lambda T_\tau \calL x = e^{-s\tau} \calL x.
\]
This is called the \term{time-shift property} of the Laplace transform.  The results also holds under certiain conditions when $H$ is a differentiator, that is,
\[
\calL D x = \lambda D \calL x = s \calL x.
\]
This is called the \term{differentiation property} of the Laplace transform (Exercise~\ref{exer:laplacetransdiffandtimeshift}).
...
Let $H$ be a linear shift-invariant system such that $y = H(x)$ whenever $x$ and $y$ satisfy the differential equation~\eqref{eq:diffinlapsection}.   According to~\eqref{eq:transferLaplcetheorem} the transfer function of $H$ is
\[
