...
\end{equation}
called the Fourier transform.  The Fourier transform is a function of the real number $f$ and so $\calF x \in \reals \to \complex$, that is, $\calF x$ is a signal.  For example, the rectangular pulse $\rect(t)$ from~\eqref{eq:rectfuncdefn} is absolutely integrable and has Fourier transform
\begin{align}
...
% \]
 ...
 \end{equation}
when the signals $x$ and $y$ are absolutely integrable.  In words: the Fourier transform of a convolution of signals is given by the multiplication of the Fourier transforms of those signals.  The convolution of two absolutely integrable signals is always absolutely integrable (Exercises~\ref{excer:convabsisabs}). 
...
\]
of the Fourier transform.  These results motivate assigning the following Fourier transforms to the delta ``function'', its shift $\delta(t - \tau)$, and its derivatives
\begin{equation}\label{eqftsofdeltas}
\calF \delta = 1, \qquad \calF\big(\delta(t - \tau)\big) = e^{-j2\pi\tau f}, \qquad \calL(\delta^k) = (j2\pi f)^k.
\end{equation}
...
\]
These conventions are common in the literature~\citep{Oppenheiim_sigs_sys_1996}, but must be used caution.  There is no guarantee that mechanical mathematical manipulations involving the delta-``function'' will lead to sensible results.
%  and the following
