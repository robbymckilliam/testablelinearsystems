...
%underdamped
 ...
 \def\hud(#1){\step(#1)*exp(-\alphaud*#1)*sin(\theta*#1 r)/\M/\theta}

%critically damped
 \def\alphacd{(\Bcd/\M/2)}
 \def\hcd(#1){\step(#1)*#1*exp(-\alphacd*#1)/\M}

%over damped
\def\Bod{(\Bcd*2)}
\def\alphaod{(\Bod/\M/2)}
\def\betaod{(sqrt((\Bod*\Bod) - 4*\K*\M)/\M/2.0)}
...
\newcommand{\animations}[1]{
  ...
  \begin{scope}[yscale=9]
    \draw[->] (-1,0) -- (8.5,0) node[above] {$t$};
    \draw[->] (0,-0.17) -- (0,0.47) node[right] {$h(t)$};
    \draw[thick] (-0.5,0) -- (0,0) {};
    % \draw[color=black,thick] plot[domain=-1:10,samples=201] (\x,{\step(\x)});
    \draw[color=black,thick] plot[domain=0:8.2,samples=101,smooth] (\x,{\hud(\x)});
    \pgfmathparse{\hud(1)}\pgfmathresult \let\pudoneval\pgfmathresult
    \node[right] at (1,\pudoneval) {$\tfrac{\pi}{3}$};
    \draw[color=black,thick] plot[domain=0:8.2,samples=101,smooth] (\x,{\hod(\x)});
    \pgfmathparse{\hod(0.8)}\pgfmathresult \let\podoneval\pgfmathresult
    \node[below] at (0.8,\podoneval) {$2\pi$};
    \draw[color=black,thick] plot[domain=0:8.2,samples=101,smooth] (\x,{\hcd(\x)});
    \pgfmathparse{\hcd(0.9)}\pgfmathresult \let\pcdoneval\pgfmathresult
    \node[right] at (0.9,\pcdoneval) {$\pi$};
    ...
    \htick{-0.1} node[pos=0.5,left] {$-0.1$};
    \begin{scope}
      \clip (-1,-0.5) rectangle (8.5,0.3);
  ...
  \pgfmathparse{\hod(#1)*6}\pgfmathresult \let\podval\pgfmathresult
