...
Ideal highpass and bandpass digital filters can be constructed using $L_c^P$ in much the same way that ideal analogue filters were constructed using $L_c$ in Section~\ref{sec:analogue-filters}.  An \term{ideal high pass digital filter} with cuttoff frequency $c$ and period $P = \tfrac{1}{F} < \tfrac{1}{2c}$ is the system given by the linear combination 
\[
T_0 - L_c^{P}.
\]  
...
\[
L_c^{P,w}x(t) = \sum_{m = -a}^a h_n T_{Pm}x(t) \qquad \text{where $a = \floor{FW/2}$}
\]
...
We consider again the \SI{20}{\second} sampled audio signal from Test~\ref{test:ftlecturerec}.  The mono audio recording contain $N = 440998$ samples that we denote by $c_0,c_1,\dots,c_{N-1}$ at rate $F = \SI{22050}{\hertz}$.  In Test~\ref{test:ftlecturerec} the Fourier transform of the audio signal computed was plotted in Figure~\ref{fig:test:fouriertransformlecturevideo}. It was observed that audio recording contains human voice that primarily resides at lower frequencies below \SI{5}{\kilo\hertz} and also interfering signals occuring at approximately $\pm\SI{8}{\kilo\hertz}$ and between \SI{4900}{\hertz} and \SI{5900}{\hertz}.  The interference is audible in the recording as an annoying high pitched hum.  In this test, a low pass digital finite impulse response filter will be used to $L_{c}^{P,w}$ remove this hum.

It will be convenient to choose period of the filter to be $P = \tfrac{1}{F}$ where $F = \SI{22050}{\hertz}$ is the sample rate of the audio.  The cuttoff frequency is chosen to be $c = \SI{4800}{\hertz}$ and a Blackman window~\eqref{eq:blackmanwindow} of width $W=\tfrac{10}{c} = \tfrac{1}{480}$ is used.  This filter has impulse response
\[
h_n = 2 c P w(t) \sinc( 2c P n)
\]
...
\[
a = \floor{\frac{FW}{2}} = \floor{\frac{735}{32}} = 22.
\] 
This filter has $2a+1 = 45$ taps.
