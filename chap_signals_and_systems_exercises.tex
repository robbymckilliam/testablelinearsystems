...
\item \label{excer:integratornotstable1} Show that the integrator $I_a : L_{\text{loc}} \to \reals \to \complex$ for any finite $a\in\reals$ is not stable.
\begin{solution}
...
\item \label{excer:domainintegrator} Show that if the signal $x$ is locally integrable and $\int_{-\infty}^0\abs{x(t)}dt < \infty$ then $I_\infty x(t) = \int_{-\infty}^t\abs{x(t)}dt < \infty$ for all $t \in \reals$.
