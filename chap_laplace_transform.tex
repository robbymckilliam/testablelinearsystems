...
\]
We take $\roc(x)$ as the domain of $\calL x$ and so $\calL x \in \roc(x) \to \complex$, that is, the Laplace transform is a complex valued function of the region of convergence.

For example, the Laplace transform of the right sided signal $e^{ \alpha t}u(t)$ has region of convergence given by all those complex numbers with real part greater than the real part of $\alpha$, that is,
\[
...
Now suppose that $H_1$ and $H_2$ are also regular systems with impulse responses $h_1$ and $h_2$. We showed in Section~\ref{sec:line-comb-comp} that the composition $H = H_2 H_1$ is a regular system with impulse response given by the convolution $h = h_1 * h_2$.  Because,
\[
...
In Chapter~\ref{sec:syst-modell-diff} we derived differential equations of the form
\begin{equation}\label{eq:diffinlapsection}
\sum_{\ell=0}^{m} a_\ell D^\ell x = \sum_{\ell=0}^{k} b_\ell D^\ell y,
\end{equation}
to model electrical, mechanical, and electromechanical devices. %  Taking Laplace transforms of both sides of this equation,
% \begin{align*}
...
Suppose that $H \in X \to Y$ is a linear shift-invariant system such that $y = Hx$ if $x$ and $y$ satisfy such a differential equation.  Since $H$ is linear and shift-invariant the response of $H$ to the complex exponential signal $e^{st}$ satisfies $He^{st} = \lambda H(s) e^{st}$.  Substituting $x(t) = e^{st}$ and $y = \lambda H(s) e^{st}$ into the differential equation gives
\[
\sum_{\ell=0}^{m} a_\ell s^{\ell} e^{st} = \sum_{\ell=0}^{m} a_\ell s^{\ell} \lambda H(s) e^{st} = \lambda H(s) \sum_{\ell=0}^{m} a_\ell s^{\ell} e^{st}
\]
