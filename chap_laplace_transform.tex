...
We showed in Section~\ref{sec:line-comb-comp} that if $H_1 \in X \to Y$ and $H_2 \in Y \to Z$ are regular systems with impulse responses $h_1$ and $h_2$, then the impulse response of the composition $H = H_2 H_1$ is given by the convolution $h = h_1 * h_2$.  Because,
\[
...
\end{equation}
that holds on the intersection of the regions of convergence of $\calL x$ and $\calL y$, that is, $\roc(x) \cap \roc(y)$.  In words: the Laplace transform of a convolution of signals is the multiplication of their Laplace transforms.
...
\end{equation}
Thus, the Laplace transform of the output signal $y = H x$ is the transfer function of the system $H$ multiplied by the Laplace transform of the input signal $x$.  This result also holds for the shifter, that is,
\[
\calL T_{\tau} x = \lambda T_\tau \calL x = e^{-s\tau} \calL x \qquad x \in \reals \to \complex.
\]
This is called the \term{time-shift property} of the Laplace transform.  The results also holds for the differentiator, that is,
\[
\calL D x = \lambda D \calL x = s \calL x.
\]
under the added assumption that  BLERG.  This is called the \term{differentiation property} of the Laplace transform (Exercise~\ref{exer:laplacetransdiffandtimeshift}).
