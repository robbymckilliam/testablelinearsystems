...
\def\hod(#1){ifthenelse(#1>0,((exp((\betaod-\alphaod)*#1)-1)/(\betaod-\alphaod)+(exp(-(\betaod+\alphaod)*#1)-1)/(\betaod+\alphaod))/\M/\betaod/2, 0)}
...
\newcommand{\animations}[1]{
  ...
  \begin{scope}[yscale=\scaley,xscale=\scalex]
    ...
    \draw[color=black,thick] plot[domain=0:10.1,samples=101,smooth] (\x,{\hud(\x)});
    \pgfmathparse{\hud(1.7)}\pgfmathresult \let\pudoneval\pgfmathresult
    \node[left] at (1.7,\pudoneval) {$\tfrac{\pi}{3}$};
    \draw[color=black,thick] plot[domain=0:10.1,samples=101,smooth] (\x,{\hod(\x)});
    \pgfmathparse{\hod(2.5)}\pgfmathresult \let\podoneval\pgfmathresult
    \node[above] at (2.5,\podoneval) {$2\pi$};
    \draw[color=black,thick] plot[domain=0:10.1,samples=101,smooth] (\x,{\hcd(\x)});
