...
Because the signal $x$ has bandwidth $b = \tfrac{7}{4}$ the spectrum of the filter for frequencies $\abs{f} > \tfrac{7}{4}$ is not important.  By exploting this property we will find that the same lowpass filtering can be achieved with a discrete time system.  Denote by $L_{c}^P$ the discrete time system with sample period $P = \tfrac{1}{F}$ and discrete impulse response $h_n = 2cP \sinc(2c Pn)$.  That is, $h$ is given by sampling with period $P$ and rescaling by $P$, the impulse response of the ideal low pass filter $L_c$.  From~\eqref{eq:dftsignalperiodicsumP}, the discrete time Fourier transform of $h$ is given by periodic summation of $\rect(Ff/2c)$, that is,
\[
...
\end{equation}
The spectrum is periodic with period $F$ and is plotted by the shaded region in Figure~\ref{fig:bandlimiteddiscretetimefiltering} in the case that $c = 1$ and $F = 4$.  The system $L_c^P$ is called an \term{ideal lowpass digital filter} with cuttoff frequency $c$ and sample period $P$.  If $c \in (\tfrac{1}{2}, \tfrac{5}{4})$ and $F > \tfrac{7}{2}$ then the response of $L_c^P$ to input signal $x$ from~\eqref{eq:examplebandlimtedsignalforfiltering} has Fourier transform
\[
...
An ideal high pass digital filter with cuttoff frequency $c$ and period $P$ is the system $T_0 - L_c^{P}$.  The impulse response is $\delta_n - 2cP\sinc(2cnP)$.  The spectrum $1 - \Lambda L_c^{P}$ is plotted in Figure~\ref{fig:bandlimiteddiscretetimefiltering} (middle) in the case that $c = 1$ and $F = \tfrac{1}{P} = 4$.  The Fourier transform of the response of $T_0 - L_c^P$ to the input signal $x$ with Fourier transform~\eqref{eq:examplebandlimtedsignalforfiltering} is 
\[
\calF( T_0 x - L_c^P x ) = (\Lambda T_0 - \Lambda H) \, \calF x = \hat{x} - \tfrac{4}{3} \rect(f) = \cos(2\pi f) \big( \rect(2f - 3) + \rect(2f + 3) \big)
\]
This response is plotted in Figure~\ref{fig:bandlimiteddiscretetimefiltering} (middle).  The two raised cosine pulses are preserved and the rectangular pulse at the origin is removed.  An ideal bandpass digital filter with upper cuttoff frequency $u$ and lower cuttoff frequency $\ell$ is the system $L_u^P - L_\ell^P$.  The impulse response is the sequence  $2uP\sinc(2unP) -  2 \ell P\sinc(2\ell nP)$.  The spectrum is plotted in Figure~\ref{fig:bandlimiteddiscretetimefiltering} (bottom) in the case that $u = \tfrac{3}{2}$, $\ell = \tfrac{1}{4}$, and $F = \tfrac{1}{P} = 4$.

The conclusion that the spectrum of $L_{c}^P$ satisfies~\eqref{eq:LambdaHidealklowpassdigitalsceptical} needs to be taken with some scepticism because the discrete impulse response $h_n = 2cP\sinc(2c P n)$ is not absolutely summable and so the domain $\cep \dom_P h$ does not contain the imaginary axis.  Like ideal analogue filters (Section~\ref{sec:analogue-filters}), ideal digital filters will not be useful in practice and so this caveat will not affect the results that follow. The concept of ideal filters is nonetheless a valuable aid to intuition.  

We now turn our attention to practical digital filters.  The first of these are called \term{finite impulse response filters} or \term{FIR filters} for short.  One feature preventing practical implementation of the ideal lowpass digital filter is that the discrete impulse response $h_n = 2c\sin(2cPn)$ is not of finite support.  Computing the response $\sum_{m \in \ints} h_m T_{Pm} x(t)$ for any time $t \in \reals$ might require summing an infinite number of terms.  A pragramtic approach to alleviate this problem is to first approximate the ideal analogue lowpass filter by one with impulse response
\[
2c w(t) \sinc(2ct)
\]
where $w(t)$ is a bounded signal called a \term{window function} or \term{apodization function}.  The window function $w(t)$ is zero outside an interval $(-W/2, W/2)$ where $W > 0$ is called the \term{window width}.  Because of the window function this impulse response has finite support and is absolutely integrable.  Common window functions include the rectangular window $w(t) = \rect(t/W)$, the triangular (or \term{Bartlett}) window $w(t) = \bigtriangleup(2t/W)$ where 
\[
...
A low pass finite impulse response filter with period $P$, cuttoff frequency $c$, and window $w$ is a discrete time system $L_{c,P,w}$ with period $P$ having impulse response $h_n = 2c w(nP) \sinc(2c nP)$ given by sampling the windowed sinc function.  Becasuse $w(t)$ is zero outside the interval $(-W/2, W/2)$ the impulse response $h_n = 0$ when $n P \notin (-FW/2, FW/2)$ and only a finite number of elements of $h$ are nonzero.  For this reason, the response of $L_{c,P,w}$ to any input signal $x$ can be computed by the finite sum
\[
L_{c,P,w}x(t) = \sum_{m = -a}^a h_n T_{Pm}x(t) \qquad \text{where $a = \floor{FW/2}$}
\]
and $\floor{FW/2}$ denotes the greatest integer less than or equal to $FW/2$.  The spectrum of this filter is given by periodic summation of the Fourier transform $\hat{w}(f) * \rect(f/2c)$ of the windowed sinc function, that is,
\[
\Lambda L_{c,P,w}(f) = \sum_{m \in \ints} \big(\hat{w}(t) *  \rect(f/2c) \big)(Ff + Fm)
\]
Now consider filtering the signal $x$ from~\eqref{eq:examplebandlimtedsignalforfiltering} using a low pass finite impulse response filter $L_{c,P,w}$.  Figure~\ref{eq:blackmanwindow} shows  the response in the case that the cuttoff frequency $c = 1$, the sample rate $F = \tfrac{1}{P} = 4$, and for the rectangular window function $w(t) = \rect{t/W}$ with width $W = 3$ and $W = 8$ and with a Blackman window function with $W = 8$.
...
%      \draw[idealowpass] (-4.3,0) -- (-\c,0) -- (-\c,1) -- (\c,1) -- (\c,0) -- (4.3,0) -- cycle;
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Fourier transform $\tfrac{4}{3}\rect(f) - \cos(2\pi f) \big( \rect(2f - 3) + \rect(2f + 3) \big)$ of a bandlimited signal with bandwidth $b \leq \tfrac{7}{4}$ (dashed). The solid line shows the magnitude of the Fourier transform of the response to this signal of the ideal lowpass digital filter with cuttoff $c=1$ and sample rate $F = \tfrac{1}{P} = 4$.  %The spectrum of this filter is shown by the hatched rectangular pulse at the origin.  
