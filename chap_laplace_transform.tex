...
\end{equation}
called the \term{Laplace transform} of $x$.  The Laplace transform $\calL x(s)$ is not necessarily defined for all complex numbers $s \in \complex$.  
Let $R_x$ be the set of complex numbers $s$ such that $x(t) e^{-s t}$ is absolutely integrable, that is,
\[
\int_{-\infty}^{\infty} \abs{x(t) e^{-s t}} dt < \infty  \qquad \text{if and only if $s \in R_x$}.
\]
The set $R_x \subseteq \complex$ is called the \term{region of convergence} of the signal $x$.  The Laplace transform $\calL x(s)$ is finite for all $s \in R_x$ because 
\[
\sabs{\calL x(s)} = \abs{\int_{-\infty}^{\infty} x(t) e^{-st} dt} \leq \int_{-\infty}^{\infty} \abs{x(t) e^{-s t}} dt < \infty
\]
for all $s \in R_x$.  We take $R_x$ as the domain of $\calL x$ and so $\calL x \in R_x \to \complex$.  That is, the Laplace transform of a signal is a complex valued function of the region of convergence of that signal.
...
In the previous example the Laplace transform is not finite for any $s$ if $\Re(\alpha) \geq \Re(\beta)$.  The region of convergence is the empty set.  Other signals also have this property.  For example, the signal $x(t) = 1$ because
\[
