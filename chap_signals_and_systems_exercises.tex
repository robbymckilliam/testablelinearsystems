...
\item \label{exer:L1L2linearshiftinvariant} Show that the set of absolutely integrable signals $L^1$ and the set of square integrable signals $L^2$ are linear space and that they are shift-invariant. 
...
\item State whether the system $H(x) = 1$ is linear, shift-invariant,  stable.
\begin{solution}
It is not linear because for any signal $x$ and real number $a \neq 1$
\[
H(ax) = 1 \neq  aH(x) = a.
